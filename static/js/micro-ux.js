...
(function () {
  ...
  /* One filter row, the School Directory's (owner, 2026-09-25). A row of
     three or more filters is marked `edify-filter-row`; on a desktop
     interactions.css shares the width between its fields on one line. Past
     its slots (six wide, five narrow) the last slot becomes "More filters",
     a panel the remaining fields move into, still inside their form. Below
     a desktop an automatic panel gives its fields back. A template may
     author its own `data-edify-filter-more`; `data-edify-filter-row="off"`
     opts a row out. The Directory, page headers and the oversight period
     bar keep their own layout. */
  ...
  /* The row holds most of the bar's field shells; fields tucked in a panel,
     dialog or collapsed section are not the row's. */
  function filterRowOf(bar) {
    ...
    bar.querySelectorAll(FILTER_CONTROL).forEach(function (control) {
      ...
      if (shell === bar || !shell.parentElement) return;
      var list = counts.get(shell.parentElement) || [];
  ...
  /* Applied = moved off the first choice ("All" or the page's default). */
  function filterMoreCount(more) {
    ...
    more.querySelectorAll('[data-edify-filter-more-panel] :is(' + FILTER_CONTROL + ')').forEach(function (field) {
      ...
      if (!field.disabled && value && value.toLowerCase() !== 'all' && (field.tagName !== 'SELECT' || field.selectedIndex > 0)) count += 1;
    });
  ...
  function arrangeFilterRows(root) {
    ...
    elementsWithin(root, FILTER_CONTAINERS).forEach(function (bar) {
      if (bar.closest('.school-filters-form, .school-filter-canvas, .edify-page-header, .oversight-period-filter, [role="dialog"], .drawer-body, [data-edify-filter-row="off"]')) return;
      ...
      var slots = desktopFilterRows.matches ? (wideFilterRows.matches ? 6 : 5) : 0;
      /* Moving a field is a mutation this file observes: a row already laid
         out for this width is left alone. */
      if (auto) {
        if (String(slots) === auto.dataset.slots) return;
        Array.from(auto.lastElementChild.children).forEach(function (field) { row.insertBefore(field, auto); });
        auto.remove();
      ...
      if (shells.length + (authored ? 1 : 0) < 3) return;
      ...
      if (authored) return filterMoreCount(authored);
      ...
      var overflow = slots && shells.length > slots ? movable.slice(Math.max(0, movable.length - shells.length + slots - 1)) : [];
      ...
      more.dataset.slots = String(slots);
      more.innerHTML = '<button type="button" class="edify-filter-more__toggle" data-edify-filter-more-toggle aria-expanded="false" aria-controls="' + id + '">'
        ...
        + '<svg class="edify-filter-more__chevron" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 9l-7 7-7-7"/></svg></button>'
      ...
      row.insertBefore(more, kept.length ? kept[kept.length - 1].nextSibling : row.firstChild);
      overflow.forEach(function (shell) { more.lastElementChild.appendChild(shell); });
  ...
  function toggleFilterMore(more, open) {
    ...
    if (open) {
      ...
      var first = panel.querySelector(FILTER_CONTROL);
