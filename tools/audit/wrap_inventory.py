...
urls = json.loads((OUT / "role_urls.json").read_text())
EMAILS = {
    "cd": "cd@edify.org",
    "pl": "pl1@edify.org",
    "ia": "ia@edify.org",
...
SHOTS = {
    ("cd", "/analytics/country-director"): ".edify-section-nav__inner",
    ("cd", "/fund-requests/weekly"): "nav[data-edify-tablist]",
    ("cd", "/staff"): "main table",
    ("cd", "/cost-settings"): "main table",
    ("cd", "/dashboard?view=operations"): "table.edify-table--truncate",
    ("cd", "/work-plan"): "main table",
    ("pl", "/partners"): "main table",
    ("ia", "/ia/verification/"): "main table",
    ("pl", "/leave/team-availability"): "main table",
    ("cd", "/debriefs"): "main table",
}
PROBE = r"""() => {
...
with sync_playwright() as p:
    ...
    for role in sys.argv[1:] or ["cd", "pl", "ia", "accountant"]:
        pages = list(
            dict.fromkeys(
                (["/dashboard?view=operations"] if role in ("cd", "pl") else [])
                + urls.get(EMAILS[role], [])
            )
        )
        ...
        for url in pages:
            try:
                ...
                if "/login" in page.url or "policy" in page.url:
                    continue
                r = page.evaluate(PROBE)
                if r["rails"] or r["tables"] or errors:
                    print(
                        role,
                        url,
                        "| rails",
                ...
                if sel:
                    ...
                    if el:
                        el.screenshot(
                            path=f"{S}/after_{role}_{url.strip('/').replace('/', '_').replace('?', '_').replace('=', '_') or 'home'}.png"
                        )
            except Exception as e:
                print(role, url, "ERR", str(e)[:80])
        ctx.close()
    b.close()
