...
const md = [];
// A Markdown table cell: backslashes first (so an escape the value already
// carries cannot swallow the one added for a pipe), then pipes, then line
// breaks, which would end the row.
