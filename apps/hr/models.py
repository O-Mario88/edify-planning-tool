...
from apps.core.models import CuidField, TimeStampedModel, SoftDeleteModel
...
class Vacancy(TimeStampedModel):
    ...
    reporting_manager = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="managed_vacancies"
    )
    employment_type = models.CharField(max_length=64, default="Full-time") # Full-time, Part-time, Contract, etc.
    reason_for_vacancy = models.TextField(null=True, blank=True)
    replacement_or_new_role = models.CharField(
        max_length=32, choices=[("replacement", "Replacement"), ("new_role", "New Role")], default="new_role"
    )
    ...
    status = models.CharField(max_length=32, default="Draft") # Draft, Pending Approval, Approved, Open, Screening, etc.
    approved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="approved_vacancies"
    )
...
class Application(TimeStampedModel):
    ...
    stage = models.CharField(max_length=64, default="Applied") # Applied, Screened, Shortlisted, Interview 1, Interview 2, Assessment, Reference Check, Recommended, Offer, Hired, Rejected, Withdrawn, Talent Pool
...
class OnboardingPlan(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.OneToOneField(StaffProfile, on_delete=models.CASCADE, related_name="onboarding_plan")
    status = models.CharField(max_length=32, default="Not Started") # Not Started, Documents Pending, Account Setup Pending, Orientation Pending, Role Training Pending, Supervisor Review Pending, Ready for Activation, Active, Overdue
...
class EmployeeComplianceRecord(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="compliance_records")
    requirement = models.ForeignKey(ComplianceRequirement, on_delete=models.CASCADE)
    status = models.CharField(max_length=32, default="Missing") # Compliant, Due Soon, Expired, Missing, Under Review, Exception Approved
    document_url = models.CharField(max_length=512, null=True, blank=True)
    expiry_date = models.DateField(null=True, blank=True)
    verified_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="verified_compliance_records"
    )
...
class PerformanceReview(TimeStampedModel):
    ...
    period = models.CharField(max_length=32, db_index=True) # e.g. "FY 2024/25", "May 2025"
    review_type = models.CharField(max_length=64, default="Quarterly Review") # Quarterly Review, FY Review, Probation Review
    status = models.CharField(max_length=32, default="Not Started") # Not Started, Self-Assessment Pending, Manager Review Pending, Calibration Pending, HR Review Pending, Completed, Closed
    due_date = models.DateField()
    rating = models.CharField(max_length=32, null=True, blank=True) # Strong (70-100%), Fair (50-69%), At Risk (<50%)
...
class PerformanceImprovementPlan(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="pip_plans")
    status = models.CharField(max_length=32, default="Draft") # Draft, Active, Progress Review, Successfully Completed, Escalated, Closed
    cause = models.CharField(max_length=128) # planning gap, execution gap, conduct issue, attendance issue, skills gap, etc.
...
class CPDAssignment(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="cpd_assignments")
    course_name = models.CharField(max_length=255)
    category = models.CharField(max_length=64) # Leadership, Program Quality, School Improvement, Safeguarding, Compliance, Digital Systems
    status = models.CharField(max_length=32, default="Assigned") # Recommended, Assigned, Enrolled, In Progress, Completed, Verified, Expired
...
class EmployeeSkill(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="skills_matrix")
...
class SuccessionCandidate(TimeStampedModel):
    ...
    staff_successor = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="succession_nominations")
    readiness = models.CharField(max_length=64, default="Development Required") # Ready Now, Ready in 6-12 Months, Ready in 1-2 Years, Development Required, Not Currently Ready
...
class EmployeeRelationsCase(TimeStampedModel):
    ...
    id = CuidField()
    case_type = models.CharField(max_length=64, db_index=True) # Grievance, Conflict, Harassment Concern, Conduct Concern, Whistleblowing, Safeguarding Referral
    severity = models.CharField(max_length=32, default="medium") # low, medium, high, critical
    status = models.CharField(max_length=32, default="Submitted") # Submitted, Triage, Under Review, Investigation, Resolved, Closed
    case_owner = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="relations_cases_assigned"
    )
...
class CompensationRecord(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.OneToOneField(StaffProfile, on_delete=models.CASCADE, related_name="compensation_details")
    ...
    status = models.CharField(max_length=32, default="HR Review") # HR Review, Approved, etc.
...
class PayrollReadinessRecord(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.ForeignKey(StaffProfile, on_delete=models.CASCADE, related_name="payroll_checks")
...
class OffboardingPlan(TimeStampedModel):
    ...
    id = CuidField()
    staff = models.OneToOneField(StaffProfile, on_delete=models.CASCADE, related_name="offboarding_details")
    status = models.CharField(max_length=32, default="Initiated") # Initiated, Portfolio Reassigned, Equipment Returned, Finance Cleared, Closed
    last_working_day = models.DateField(null=True, blank=True)
    handover_owner = models.ForeignKey(
        StaffProfile, on_delete=models.SET_NULL, null=True, blank=True, related_name="assigned_handovers"
    )
