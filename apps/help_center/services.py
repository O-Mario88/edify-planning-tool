...
def canonical_specs() -> list[dict]:
    ...
    specs.append(
        _article_spec(
            title="Getting Started with Edify",
            slug="getting-started",
            category="Getting Started",
            roles=[role for role, *_ in []]
            or [
                "CCEO",
                "Program Lead",
                "CountryDirector",
            ...
            keywords=["orientation", "workflow", "help", "manual"],
            feature="help",
            workflow="ecosystem",
            steps=[
                "Choose your active role and use the role guide matched to it.",
                "Start from the source record: School, Cluster, Activity, fund request or people record.",
                "Use the next-action/status on that record; do not infer completion from a dashboard alone.",
                "Use Contextual Help on any workspace to open the relevant article.",
            ],
        )
    )
    ...
    for title, slug, category, roles, routes, sources, summary, first_step in FEATURES:
        specs.append(
            _article_spec(
                ...
                steps=[
                    first_step,
                    "Use the controls on this page and fix any message shown before you submit.",
                    "Afterward, reopen or refresh the same record and check its status and who needs to act next.",
                ],
                feature=slug,
            )
        )
    for title, slug, correction in TROUBLESHOOTING:
        specs.append(
            _article_spec(
                ...
                steps=[
                    "Read the message or status shown on the record.",
                    correction,
                    "If someone else must fix it, use the named owner, notification or message without sharing private information.",
                ],
    ...
    specs.append(
        _article_spec(
            ...
            slug="glossary-workflow-statuses",
            category="Glossary",
            roles=[
                "CCEO",
                "Program Lead",
            ...
            steps=[
                "Read the status on the source record.",
                "Use the related Contextual Help article for the enabled next action.",
                "Treat a returned state as correction work, not a completed result.",
            ],
    ...
    specs.append(
        _article_spec(
            title="Edify Identifiers: School, Salesforce, NetSuite and Finance References",
            slug="glossary-identifiers",
            category="Glossary",
            roles=[
                "CCEO",
                "Program Lead",
            ...
            steps=[
                "Use School ID to match the structured SSA CSV row to the directory school.",
                "Enter a Salesforce Activity ID on the completed activity, using SVE- for visits and TS- for trainings.",
                "Enter a NetSuite Expense ID after accountability; the Accountant verifies it.",
                "Keep payment, return and reimbursement references on their original finance record.",
            ],
            feature="identifiers",
        )
    )
...
def ensure_canonical_content() -> dict:
    ...
    for spec in canonical_specs():
        article, was_created = HelpArticle.objects.get_or_create(
            ...
            defaults={
                ...
                "estimated_reading_minutes": max(
                    ...
                    min(
                        12,
                        (len(str(spec["content"])) + len(spec["summary"])) // 500 + 2,
                    ),
                ),
            },
        )
        ...
        HelpArticleRoleAccess.objects.bulk_create(
            [
                HelpArticleRoleAccess(article=article, role=role)
        ...
        HelpSearchKeyword.objects.bulk_create(
            [
                HelpSearchKeyword(article=article, term=term)
        ...
        HelpArticleVersion.objects.create(
            ...
            change_summary="Initial canonical-service reviewed publication.",
            reviewer_name=CANONICAL_REVIEWER,
            reviewed_at=now,
        )
    _seed_glossary()
...
def _seed_glossary() -> None:
    status_article = HelpArticle.objects.filter(
        slug="glossary-workflow-statuses"
    ).first()
    ...
    for term, definition, used_in, related in GLOSSARY:
        HelpGlossaryTerm.objects.get_or_create(
            ...
            defaults={
                ...
                "article": identifiers
                if "ID" in term or "Reference" in term
                else status_article,
            },
        )
...
def _seed_walkthroughs() -> None:
    walkthroughs = [
        (
            ...
            "planning",
            [
                "Choose a school and delivery details.",
                "Resolve route, date and Cost Catalogue blockers.",
                "Confirm the saved activity in My Plan.",
            ],
        ),
        (
            "Upload Evidence",
            ...
            "evidence",
            [
                "Open the assigned activity.",
                "Upload an accepted evidence file.",
                "Inspect the preview and submit for review.",
            ],
        ),
        (
            "Enter Salesforce ID",
            "enter-salesforce-id",
            "feature-salesforce-activity-ids",
            "my-plan",
            [
                "Open the completed activity.",
                "Enter the correctly prefixed unique ID.",
                "Confirm the next IA status.",
            ],
        ),
        (
            "Complete Accountability",
            "complete-accountability",
            "feature-accountability-returns-reimbursements",
            "accounts/accountability/",
            [
                "Enter real spend and receipt information.",
                "Explain variance.",
                "Submit the NetSuite reference when ready.",
            ],
        ),
        (
            "Review Evidence as IA",
            "review-evidence-ia",
            "feature-ia-verification",
            "ia/verification/",
            [
                "Open a record awaiting IA.",
                "Complete each checklist check.",
                "Clear or return with reasons.",
            ],
        ),
        (
            "Set a Cost Catalogue Rate",
            "set-cost-catalogue-rate",
            "feature-cost-catalogue",
            "cost-settings",
            [
                "Confirm the cost item and effective date.",
                "Enter whole-UGX rate.",
                "Save before planners create new activities.",
            ],
        ),
    ]
...
def visible_articles(role: str | None):
    queryset = (
        ...
        .select_related("category", "reviewer")
        .prefetch_related("role_accesses", "related_articles")
    )
    if role:
        queryset = queryset.filter(
            Q(role_accesses__role=role) | ~Q(role_accesses__isnull=False)
        ).distinct()
    return queryset
...
def personalized_articles(role: str, limit: int = 6):
    ...
    priorities_by_role = {
        "CCEO": [
            "feature-planning",
            "feature-my-plan",
            "feature-ssa-comparison-recommendations",
            "workflow-evidence-upload",
            "feature-salesforce-activity-ids",
            "feature-accountability-returns-reimbursements",
        ],
        "ImpactAssessment": [
            "feature-ia-verification",
            "feature-evidence-pdf-viewer",
            "feature-ssa-upload-history",
            "feature-unmatched-ssa",
            "workflow-ia-clear-return",
        ],
        "Accountant": [
            "feature-disbursement",
            "feature-accountability-returns-reimbursements",
            "workflow-netsuite-verification",
            "feature-partner-payments",
            "feature-completed-activities",
        ],
        "Program Lead": [
            "feature-my-plan",
            "feature-targets",
            "workflow-cceo-fund-approval",
            "workflow-catch-up-plans",
            "feature-field-debrief-leadership-actions",
        ],
        "CountryDirector": [
            "feature-cost-catalogue",
            "feature-monthly-annual-budgets",
            "workflow-country-budget-rvp",
        ...
        "RegionalVicePresident": [
            ...
            "feature-monthly-annual-budgets",
            "feature-analytics-reports",
            "workflow-debrief-leadership-action",
        ],
        "HumanResources": [
            "feature-leave-temporary-coverage-feature",
            "feature-professional-development-feature",
            "feature-performance-reviews",
            "workflow-performance-review",
        ],
        "PartnerAdmin": [
            "feature-partners",
            "workflow-partner-activity-execution",
            "workflow-evidence-upload",
            "feature-salesforce-activity-ids",
            "feature-partner-payments",
        ],
        ...
        "ProjectCoordinator": [
            ...
            "feature-my-plan",
            "workflow-special-project-planning",
            "workflow-evidence-upload",
            "feature-targets",
        ],
        "Admin": [
            ...
            "feature-system-health-audit-log",
    ...
    return (
        found
        + list(articles.exclude(id__in=seen).order_by("-last_reviewed_at")[:limit])
    )[:limit]
...
def search_articles(query: str, role: str | None):
    ...
    for term in terms:
        filters |= (
            ...
            | Q(search_document__icontains=term)
            | Q(search_keywords__term__icontains=term)
        )
    return (
        visible_articles(role).filter(filters).distinct()
        if query.strip()
        else visible_articles(role).none()
    )
...
def excerpt(article: HelpArticle, query: str, limit: int = 180) -> str:
    ...
    return (
        ("…" if beginning else "")
        + text[beginning:ending].replace("\n", " ").strip()
        + ("…" if ending < len(text) else "")
    )
...
ROUTE_PREFIX_ARTICLES = [
    ...
    ("schools", "feature-school-directory"),
    ("clusters", "feature-clusters"),
    ("ssa", "feature-ssa-upload-history"),
    ("core-schools", "feature-core-schools"),
    ("planning", "feature-planning"),
    ("my-plan", "feature-my-plan"),
    ("calendar", "feature-routes-daily-visit-batches"),
    ("fund", "feature-weekly-fund-requests"),
    ("budget", "feature-monthly-annual-budgets"),
    ("cost-settings", "feature-cost-catalogue"),
    ("disbursements", "feature-disbursement"),
    ("accounts", "feature-accountability-returns-reimbursements"),
    ("activities/closure", "feature-completed-activities"),
    ("evidence", "feature-evidence-pdf-viewer"),
    ("ia", "feature-ia-verification"),
    ("partner", "feature-partners"),
    ("projects", "feature-special-projects"),
    ("targets", "feature-targets"),
    ("analytics", "feature-analytics-reports"),
    ("reports", "feature-analytics-reports"),
    ("debriefs", "feature-field-debrief-leadership-actions"),
    ("leave", "feature-leave-temporary-coverage-feature"),
    ("professional", "feature-professional-development-feature"),
    ("performance", "feature-performance-reviews"),
    ("admin", "feature-system-health-audit-log"),
    ("system", "feature-system-health-audit-log"),
...
def contextual_article(
    path: str, role: str | None, workflow_status: str = ""
) -> HelpArticle | None:
    ensure_canonical_content()
    try:
    ...
    if workflow_status:
        contexts = contexts.filter(
            Q(workflow_status=workflow_status) | Q(workflow_status="")
        )
...
def record_feedback(
    *,
    ...
    return HelpArticleFeedback.objects.create(
        ...
        user_role=getattr(user, "active_role", ""),
        helpful=helpful,
        feedback_type=feedback_type,
        comment=(comment or "").strip(),
        page_context=(page_context or "")[:255],
    )


TRANSITIONS = {
    HelpArticleState.DRAFT: {"submit": HelpArticleState.TECHNICAL_REVIEW},
    HelpArticleState.TECHNICAL_REVIEW: {
        "technical_review": HelpArticleState.PRODUCT_REVIEW,
        "return": HelpArticleState.DRAFT,
    },
    HelpArticleState.PRODUCT_REVIEW: {
        "product_review": HelpArticleState.APPROVED,
        "return": HelpArticleState.DRAFT,
    },
    HelpArticleState.APPROVED: {
        "publish": HelpArticleState.PUBLISHED,
        "return": HelpArticleState.DRAFT,
    },
    HelpArticleState.PUBLISHED: {
        "archive": HelpArticleState.ARCHIVED,
        "review_due": HelpArticleState.REVIEW_DUE,
    },
    HelpArticleState.REVIEW_DUE: {
        "publish": HelpArticleState.PUBLISHED,
        "archive": HelpArticleState.ARCHIVED,
    },
}
...
def transition_article(
    article: HelpArticle, action: str, actor, *, change_summary: str = ""
) -> HelpArticle:
    next_state = TRANSITIONS.get(article.state, {}).get(action)
    if not next_state:
        raise ValueError(
            f"Cannot {action.replace('_', ' ')} an article in {article.get_state_display().lower()}."
        )
    now = timezone.now()
    article.state = next_state
    if action in {"technical_review", "product_review", "publish"}:
        article.reviewer = actor
        article.reviewer_name = getattr(actor, "name", "") or getattr(
            actor, "email", ""
        )
        article.last_reviewed_at = now
    version_number = (
        article.versions.aggregate(latest=Max("version"))["latest"] or 0
    ) + 1
    if action == "publish":
    ...
    HelpArticleVersion.objects.create(
        ...
        state=article.state,
        snapshot=article_snapshot(article),
        change_summary=change_summary or action.replace("_", " ").capitalize(),
...
def create_revision(article: HelpArticle, author) -> HelpArticle:
    ...
    revision = HelpArticle.objects.create(
        title=f"{article.title} (revision)",
        slug=slug,
        summary=article.summary,
        content=article.content,
        search_document=article.search_document,
    ...
    HelpArticleRoleAccess.objects.bulk_create(
        [
            HelpArticleRoleAccess(article=revision, role=role)
            for role in article.role_accesses.values_list("role", flat=True)
        ]
    )
...
def collect_workflow_statuses() -> list[tuple[str, str, str, str]]:
    ...
    model_fields = [
        (
            "Activity",
            "apps.activities.models",
            "Activity",
            ["status", "evidence_status", "ia_verification_status", "payment_status"],
        ),
        ...
        (
            ...
            "apps.professional_development.models",
...
def documentation_drift_report() -> dict:
    ...
    mapped = set(
        HelpArticleRouteContext.objects.values_list("route_pattern", flat=True)
    )
    missing_routes = [
    ...
    unknown_statuses = [
        ...
        for source, field, value, label in collect_workflow_statuses()
        if label not in known_statuses
    ]
    broken_related = []
    for article in HelpArticle.objects.prefetch_related("related_articles"):
        if article.is_published and article.category_id is None:
            broken_related.append(
                {"article": article.slug, "issue": "published article missing category"}
            )
    overdue = list(
        HelpArticle.objects.filter(
            state__in=[HelpArticleState.PUBLISHED, HelpArticleState.REVIEW_DUE],
    ...
    no_version = [
        slug
        for slug in published.exclude(versions__isnull=False).values_list(
            "slug", flat=True
        )
    ]
    coverage = (
        round(100 * (len(inventory) - len(missing_routes)) / len(inventory), 1)
        if inventory
        else 100.0
    )
    return {
        ...
        "troubleshooting_articles": HelpArticle.objects.filter(
            feature="troubleshooting"
        ).count(),
