...
class Command(BaseCommand):
    help = (
        "Audit Knowledge Center route/status coverage and fail on documentation drift."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--sync-routes",
            action="store_true",
            help="Create missing canonical route mappings before auditing.",
        )
        parser.add_argument(
            "--strict",
            action="store_true",
            help="Return non-zero if coverage, status or publication checks fail.",
        )

    def handle(self, *args, **options):
        if options["sync_routes"]:
            self.stdout.write(
                self.style.SUCCESS(f"Route mappings: {sync_route_contexts()}")
            )
