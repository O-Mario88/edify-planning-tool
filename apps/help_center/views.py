    ...
    results = [
        {
            ...
            "reason": result_match_reason(article, query),
        }
...
def article(request, slug):
    ...
    if not article_for_slug(slug, _role(request)):
        return HttpResponseForbidden(
            "This guidance is not available for your active role."
        )
    return render(
        request,
...
def feedback(request, slug):
    ...
    messages.success(
        request,
        "Thanks — your feedback has been recorded for the documentation review queue.",
    )
...
def role_manual_export(request, role_slug):
    ...
    if role != _role(request) and not (
        getattr(request.user, "is_staff", False) or _role(request) == "Admin"
    ):
        raise PermissionDenied("You can export only your own role manual.")
    articles = visible_articles(role)
    response = render(
        request,
        "pages/help/manual_export.html",
        {"manual_title": f"{role} Manual", "articles": articles, "role": role},
    )
    response["Content-Disposition"] = (
        f'attachment; filename="edify-{role_slug}-manual.html"'
    )
    return response
...
def complete_manual_export(request):
    ...
    response = render(
        ...
        "pages/help/manual_export.html",
        {
            "manual_title": "Edify Authorised Manual",
            "articles": visible_articles(None),
            "role": "Authorised administration",
        },
    )
    response["Content-Disposition"] = (
        'attachment; filename="edify-authorised-manual.html"'
    )
    return response
...
def manage_article(request, slug=None):
    ...
    if instance and instance.state != HelpArticleState.DRAFT:
        messages.error(
            request,
            "Published and in-review articles are immutable. Create a revision draft so the live manual stays available during review.",
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            HelpArticleRoleAccess.objects.bulk_create(
                [
                    HelpArticleRoleAccess(article=item, role=role)
            ...
            HelpArticleRouteContext.objects.bulk_create(
                [
                    HelpArticleRouteContext(
                        article=item, route_pattern=route.strip(), priority=10
                    )
                    for route in form.cleaned_data["route_patterns"].splitlines()
                    if route.strip()
                ]
            )
            messages.success(
                request,
                "Draft saved. Submit it through technical and product review before publishing.",
            )
            return redirect("frontend:help_manage")
    else:
        form = HelpArticleForm(instance=instance)
    return render(
        request, "pages/help/edit_article.html", {"form": form, "article": instance}
    )
...
def manage_revision(request, slug):
    ...
    messages.success(
        request,
        "A separate revision draft was created. The published article remains visible until you publish and archive the predecessor.",
    )
...
def manage_transition(request, slug, action):
    ...
    try:
        ...
        messages.success(
            request,
            f"Article {action.replace('_', ' ')} completed with an auditable version entry.",
        )
    return redirect("frontend:help_manage")
