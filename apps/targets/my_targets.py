...
ANNUAL_FALLBACK = {
    ...
    "cluster_trainings": lambda tp: (tp.trainings_target or 0) + (tp.group_trainings_target or 0),
    ...
    def rebuild(user, fy: str) -> None:
        ...
        def upsert(area_key, source_type, source_id, when: date, status: str):
            ...
            row, created = TargetAchievementLedger.objects.get_or_create(
                user_id=user.id, area=areas[area_key],
                source_type=source_type, source_id=str(source_id),
                defaults={
                    "activity_date": when, "fy": fy,
            ...
            if not created and (
                ...
                row.save(update_fields=[
                    "activity_date", "credited_month", "credited_quarter",
        ...
        for area_key, (stype, types) in AREA_SOURCES.items():
            ...
            acts = Activity.objects.filter(
                responsible_staff_id__in=ids, fy=fy,
                activity_type__in=types, deleted_at__isnull=True,
            ).exclude(planned_date__isnull=True).exclude(delivery_type="partner")
            for a in acts:
                if a.status in RETURNED_STATUSES:
                    ...
                    status = "validated" if (a.salesforce_activity_id or "").strip() else "provisional"
                else:
        ...
        ssa = SsaRecord.objects.filter(
            collected_by_user_id__in=ids, deleted_at__isnull=True,
            date_of_ssa__gte=fy_start, date_of_ssa__lt=fy_end,
        )
        for rec in ssa:
            d = rec.date_of_ssa.date() if hasattr(rec.date_of_ssa, "date") else rec.date_of_ssa
            status = "validated" if rec.verification_status == "confirmed" else "provisional"
        ...
        for row in TargetAchievementLedger.objects.filter(user_id=user.id, fy=fy).exclude(
            validation_status="reversed"
        ):
...
class MyTargetQueryService:
    ...
    def monthly_targets(user, fy: str) -> dict[str, list[int]]:
        ...
        tp = StaffTargetProfile.objects.filter(staff_id=sp_id, fy=fy).first() if sp_id else None
    ...
    # ── Status math ──────────────────────────────────────────────────────────
    @staticmethod
    def status_for(achieved: int, target: int, expected_pace: int, started: bool) -> tuple[str, str]:
        if target == 0:
    ...
    def get_page(user, fy: str | None = None, month_of_fy: int | None = None) -> dict:
        ...
        def weighted_pct(month_list) -> tuple[int, int, int]:
            ...
            tot_a = tot_t = 0
            for a in areas:
                t = span_sum(targets[a.key], month_list)
                ach = span_sum(achieved[a.key], month_list)
                tot_a += ach
                tot_t += t
                if t > 0:
                    wsum += a.weight
        ...
        def period_card(kind, label, sublabel, months, start, end, emphasize=False):
            ...
            started = today >= start
            pace = Cal.expected_pace_pct(start, end, today, user) if is_current_fy else (100 if started else 0)
            status, tone = ("Not Assigned", "neutral") if started else ("Not Started", "neutral")
            ...
            return {
                "kind": kind, "label": label, "sublabel": sublabel,
                "pct": pct, "ring": min(pct, 100), "achieved": ach, "target": tgt,
                "status": status, "tone": tone, "pace": pace,
                "current": emphasize,
            }

        m_start, m_end = Cal.month_range(fy, month_of_fy)
        cards = [period_card(
            "month", Cal.month_label(fy, month_of_fy).split()[0] + f" {Cal.month_label(fy, month_of_fy).split()[1]}",
            "Monthly", [month_of_fy], m_start, m_end, emphasize=True,
        )]
        current_quarter = Cal.quarter_of_month(month_of_fy)
        for q in QUARTERS:
            qs, qe = Cal.quarter_range(fy, q)
            cards.append(period_card(
                "quarter", q, Cal.quarter_label(fy, q),
                Cal.months_of_quarter(q), qs, qe,
                emphasize=(q == current_quarter and is_current_fy),
            ))
        fy_s, fy_e = Cal.fy_range(fy)
        cards.append(period_card(
            "fy", f"FY {int(fy) - 1}/{str(fy)[-2:]}", "Full Year",
            list(range(1, 13)), fy_s, fy_e,
        ))

        # ── Target-area cards + matrix rows ─────────────────────────────────
        pace_month = Cal.expected_pace_pct(m_start, m_end, today, user) if is_current_fy else 100
        ...
        for a in areas:
            ...
            area_cards.append({
                ...
                "target": t_m, "achieved": a_m,
                "pct": round(a_m / t_m * 100) if t_m else None,
                "status": status, "tone": tone, "spark_points": pts,
            })
            cells = [{"t": t_m, "a": a_m,
                      "pct": round(a_m / t_m * 100) if t_m else None}]
            for q in QUARTERS:
                ...
                cells.append({"t": tq, "a": aq, "pct": round(aq / tq * 100) if tq else None})
            tf = sum(targets[a.key])
            af = sum(achieved[a.key])
            cells.append({"t": tf, "a": af, "pct": round(af / tf * 100) if tf else None})
            matrix_rows.append({"label": a.label, "key": a.key, "cells": cells})

        overall_cells = [{"pct": cards[0]["pct"]}] + [
        ...
        for m in range(1, 13):
            ...
            for a in areas:
                ...
                if tf > 0:
                    wsum += a.weight
                    psum += (span_sum(achieved[a.key], range(1, m + 1)) / tf * 100) * a.weight
                    esum += (span_sum(targets[a.key], range(1, m + 1)) / tf * 100) * a.weight
            actual = round(psum / wsum) if wsum else 0
            expected = round(esum / wsum) if wsum else 0
            expected_line.append(expected)
            actual_line.append(actual if (not is_current_fy or m <= current_month) else None)

        # ── Distribution + focus ─────────────────────────────────────────────
        dist = {"On Track": 0, "At Risk": 0, "Off Track": 0, "Complete / Exceeded": 0, "Not Started": 0}
        for _a, status, _tone in assigned_statuses:
            if status in ("Complete", "Exceeded"):
                ...
                dist["Not Started"] += 1
        distribution = [
            {"label": k, "count": v} for k, v in dist.items() if v
        ]
        ...
        for card in sorted(
            ...
            if card["status"] in ("Complete", "Exceeded", "On Track"):
                continue
            reason = MyTargetQueryService._gap_reason(user, card["key"], fy, month_of_fy)
            label, url = NEXT_ACTIONS[card["key"]]
            focus.append({
                "area": card["label"], "pct": card["pct"] or 0,
                "achieved": card["achieved"], "target": card["target"],
                "status": card["status"], "tone": card["tone"],
                "reason": reason, "action_label": label, "action_url": url,
            })
        ...
        return {
            ...
            "matrix_heads": [
                {"label": Cal.month_label(fy, month_of_fy).split()[0], "sub": "Monthly"},
            ...
            "trend": {
                ...
                "current_label": MONTH_LABELS[current_month - 1 if is_current_fy else 11],
    ...
    def _pipeline(user, area_key: str, fy: str, month_of_fy: int) -> dict:
        ...
        out = {"validated": [], "pending_sf": [], "ia_pending": [], "returned": [],
               "scheduled": [], "provisional": []}
        if stype == "activity":
            acts = Activity.objects.filter(
                responsible_staff_id__in=ids, fy=fy, activity_type__in=types,
                planned_date__gte=m_start, planned_date__lt=m_end,
                deleted_at__isnull=True,
            ).exclude(delivery_type="partner").select_related("school", "cluster")
            for a in acts:
                row = {
                    "name": (a.school.name if a.school_id else (a.cluster.name if a.cluster_id else "—")),
                    "type": a.activity_type.replace("_", " ").title(),
                    "date": a.planned_date, "status": a.status.replace("_", " ").title(),
                }
                if a.status in RETURNED_STATUSES:
            ...
            for r in recs:
                row = {"name": r.school.name if r.school_id else "—", "type": "SSA",
                       "date": r.date_of_ssa, "status": r.verification_status.title()}
            ...
            for s in MostSignificantChangeStory.objects.filter(
                ...
                row = {"name": s.title, "type": "MSCS", "date": s.story_date,
    ...
    def area_drawer(user, area_key: str, fy: str, month_of_fy: int) -> dict:
        ...
        m_start, m_end = Cal.month_range(fy, month_of_fy)
        today = date.today()
        wd_left = Cal.working_days(max(m_start, today), m_end, user) if today < m_end else 0
        remaining = max(0, t - a)
        weekly_pace = round(remaining / max(1, wd_left / 5), 1) if wd_left else remaining
        return {
            "ok": True, "area": area.label, "key": area_key,
            "month_label": Cal.month_label(fy, month_of_fy),
            "target": t, "achieved": a,
            "pct": round(a / t * 100) if t else None,
            "remaining": remaining, "working_days_left": wd_left,
    ...
    def export_rows(user, fy: str) -> list[list]:
        ...
        for a in TargetArea.objects.filter(active=True).order_by("sort_order"):
            for m in range(1, 13):
                ...
                rows.append([a.label, Cal.month_label(fy, m), t, ach,
            ...
            rows.append([a.label, "FY Cumulative", t, ach,
                         round(ach / t * 100) if t else ""])
        return rows
