...
User = get_user_model()
FY, PREV = "2026", "2025"
BANNED = ("schedule", "start activity", "upload evidence", "enter sf", "enter salesforce", "start plan")
...
class CDAnalyticsTest(TestCase):
    def setUp(self):
        ...
        self.cd, _ = self._staff("cd@t.org", "Diana Amaka", EdifyRole.COUNTRY_DIRECTOR.value)
        ...
        self.act_a1_visit = self._act(self.a1_sp.id, self.sch_a1, "school_visit", "completed", sf="SV-A1")
        self.act_a1_train = self._act(self.a1_sp.id, self.sch_a1, "training", "completed", teachers=10, leaders=3)
        ...
        self.act_partner = self._act(self.a1_sp.id, self.sch_a1, "school_visit", "ia_verified", delivery="partner", partner=self.partner.id)

        # Activity-advance finance pipeline (the real country finance volume).
        self._adv(self.act_a1_visit, self.a1.id, 100_000, "pending_responsible_confirmation")
        self._adv(self.act_a1_train, self.a1.id, 50_000, "disbursed", disbursed=50_000)
        self._adv(self.act_partner, self.a1.id, 30_000, "accounted", disbursed=30_000, accounted=30_000)

        # Targets (config — achievement stays computed from real completions).
        StaffTargetProfile.objects.create(staff=self.a1_sp, fy=FY, visits_target=1, trainings_target=1)
    ...
    def _school(self, sid, district, enrollment, ssa_done):
        return School.objects.create(
            ...
            enrollment=enrollment, current_fy_ssa_status="done" if ssa_done else "not_done",
        )

    def _ssa(self, school, fy, avg, scores, status):
        rec = SsaRecord.objects.create(
            school=school, fy=fy, quarter="Q1", average_score=avg,
            verification_status=status, date_of_ssa=date(int(fy) - 1, 11, 1), uploaded_by="test",
        )
        for interv, sc in scores.items():
            SsaScore.objects.create(ssa_record=rec, intervention=interv, score=sc)
        return rec

    def _act(self, sp_id, school, atype, status, sf="", teachers=None, leaders=None, delivery="staff", partner=None):
        return Activity.objects.create(
            school=school, activity_type=atype, delivery_type=delivery, status=status,
            ...
            teachers_attended=teachers, leaders_attended=leaders, assigned_partner_id=partner,
        )
    ...
    def _adv(self, activity, user_id, amount, status, disbursed=0, accounted=0):
        ...
        line = ActivityScheduleCostLine.objects.create(
            activity=activity, cost_setting_key="transport", label="Transport",
            unit_cost=amount, amount=amount,
        )
        return AdvanceRequest.objects.create(
            activity=activity, budget_line=line, responsible_user_id=user_id, fy=FY, quarter="Q3",
            planned_date=date(2026, 4, 10), amount=amount, status=status,
            disbursed_amount=disbursed, accounted_amount=accounted,
        )
    ...
    def test_cd_does_not_get_field_planning_actions(self):
        ...
        for kind, acts in S._OVERSIGHT_ACTIONS.items():
            for a in acts:
                self.assertFalse(any(b in a.lower() for b in BANNED), f"{kind}:{a} is field execution")
    ...
    def test_cd_partner_performance_uses_verified_activities_and_ssa_delta(self):
        d = self._dash()
        row = next(r for r in d["partner_performance"]["rows"] if r["name"] == "Hope Builders Network")
    ...
    def test_cd_cluster_average_uses_all_cluster_school_ssa_scores(self):
        ...
        cl = Cluster.objects.create(name="Cluster One", district=self.dist_a, region=self.region)
        # School.save() nulls cluster_id → assign via .update() (documented fixture path).
        School.objects.filter(id__in=[self.sch_a1.id, self.sch_b1.id]).update(cluster_id=cl.id, cluster_status="clustered")
        d = self._dash()
    ...
    def test_cd_recommended_actions_generate_cd_or_pl_todos(self):
        ...
        WeeklyFundRequest.objects.filter(id=wfr.id).update(status="confirmed_for_advance")
    ...
    def test_cd_drilldowns_are_read_or_oversight_actions_not_field_execution(self):
        ...
        cases = [
            ("pl", _P(id=self.pl_a.id)), ("cceo", _P(id=self.a1_sp.id)),
            ("district", _P(id=self.dist_a.id)), ("region", _P(id=self.region.id)),
            ("partner", _P(id=self.partner.id)), ("ssa", _P(intervention="leadership")),
            ("budget", _P()), ("risk", _P(issue="no_ssa")), ("kpi", _P()),
        ]
...
class CDRefinedSpecTest(CDAnalyticsTest):
    ...
    def test_cd_has_no_field_execution_actions(self):
        ...
        for item in d["recommended_actions"]["items"]:
            ...
            self.assertFalse(any(b in item["issue"].lower() for b in BANNED), item["issue"])
    ...
    def test_cd_kpis_use_verified_workflow_data(self):
        ...
        real_completed = Activity.objects.filter(
            fy=FY, status__in=COMPLETED_STATUSES, deleted_at__isnull=True).count()
    ...
    def test_ssa_enrollment_score_separate_from_school_enrollment(self):
        ...
        rec = SsaRecord.objects.filter(school=self.sch_a1, fy=FY,
                                       verification_status="confirmed").first()
    ...
    def test_recommended_action_creates_correct_todo(self):
        ...
        for t in todos:
            self.assertIn(t["owner"], {"CD", "PL"})
            self.assertTrue(t.get("route") or t.get("action_url") or t.get("action_label"))

    def test_cd_drilldowns_preserve_filters(self):
        data = S.drilldown(self.cd, "district", {"id": self.dist_a.id},
                           fy=FY, quarter="Q3", filters={"pl": self.pl_a.id})
    ...
    def test_cd_export_respects_country_scope(self):
        ...
        self.assertNotEqual(c.get("/analytics/country-director/export").status_code, 200)
    ...
    def test_htmx_cd_endpoints_enforce_permission(self):
        ...
        for url in ("/analytics/country-director",
        ...
        self.assertEqual(
            c.get(f"/analytics/country-director/drilldown?drill=district&id={self.dist_a.id}").status_code, 200)
    ...
    def test_weighted_overall_uses_five_area_weights(self):
        ...
        self.assertFalse(TargetAchievementLedger.objects.filter(
            source_id=self.act_partner.id).exists())   # partner never double-counts
