...
def leadership_summary(principal, query: dict) -> dict:
    ...
    ssa_avg_val = SsaRecord.objects.filter(
    ...
    interventions_grouped = (
        scores.values("intervention").annotate(avg=Avg("score")).order_by("avg")
    )
    by_intervention = [
        {
            "intervention": item["intervention"],
            "average": round(item["avg"], 1) if item["avg"] is not None else 0.0,
        }
    ...
    staff_count = (
        acts.exclude(responsible_staff_id__isnull=True)
    ...
    districts = [
        {
            ...
            "total": r["total"],
            "ssaDone": r["ssa_done"],
            "coverage": round((r["ssa_done"] / r["total"] * 100), 1)
            if r["total"]
            else 0,
            "core": r["core"],
        }
...
def ssa_performance_grouped(principal, query: dict) -> dict:
    ...
    if not school_groups:
        return {
            ...
            "canGroupByCceo": principal.active_role
            in ("Admin", "CountryDirector", "Program Lead", "ImpactAssessment"),
            "interventions": [
                {"code": i.value, "label": i.label} for i in SsaIntervention
            ],
            "rows": [],
        }
    ...
    for s in scores:
        ...
        if gid in group_map:
            group_map[gid]["interventions"][intervention] = (
    ...
    for gid, gdata in group_map.items():
        ...
        if recs:
            avg_sum = sum(
                r["average_score"] for r in recs if r["average_score"] is not None
            )
...
def intervention_improvement(principal, query: dict) -> dict:
    ...
    records_curr = {
        ...
        for r in SsaRecord.objects.filter(
            school__in=schools, fy=current_fy, deleted_at__isnull=True
        ).values("id", "school_id", "average_score")
    }
    records_prev = {
        r["school_id"]: r
        for r in SsaRecord.objects.filter(
            school__in=schools, fy=prev_fy, deleted_at__isnull=True
        ).values("id", "school_id", "average_score")
    }
    ...
    for gid, gdata in group_map.items():
        total_compared = (
            gdata["schoolsImproved"]
            + gdata["schoolsDeclined"]
            + gdata["schoolsNoChange"]
        )
        improvement_rate = (
            round(gdata["schoolsImproved"] / total_compared * 100)
            if total_compared > 0
            else None
        )
        ...
        for i in SsaIntervention:
            ...
            if change is not None:
                if change > 0:
                    if (
                        best_intervention is None
                        or change > best_intervention["change"]
                    ):
                        best_intervention = {
                            "code": i.value,
                            "label": i.label,
                    ...
                    if (
                        declining_intervention is None
                        or change < declining_intervention["change"]
                    ):
                        declining_intervention = {
                            "code": i.value,
                            "label": i.label,
                            "change": change,
                        }
            ...
            if curr_avg is not None:
                if (
                    weakest_intervention is None
                    or curr_avg < weakest_intervention["currAvg"]
                ):
...
def support_ssa_correlation(principal, query: dict) -> dict:
    ...
    without_support = (
        ...
        .exclude(school__activities__fy=fy)
        .aggregate(a=Avg("average_score"))["a"]
    )
    return {
        "fy": fy,
...
def contribution_summary(principal, query: dict) -> dict:
    ...
    if lens == "team" and scope.supervised_staff_ids:
        completed_qs = completed_qs.filter(
            responsible_staff_id__in=scope.supervised_staff_ids
        )
    elif lens == "combined" and scope.supervised_staff_ids:
        completed_qs = completed_qs.filter(
            responsible_staff_id__in=list(scope.supervised_staff_ids)
            + list(scope.staff_ids or [])
        )
    ...
    reached_school_ids = list(
        verified_completed_qs.filter(school_id__isnull=False)
        .values_list("school_id", flat=True)
        .distinct()
    )
    reached_schools = School.objects.filter(
        id__in=reached_school_ids, deleted_at__isnull=True
    )
    ...
    totals_trained = verified_completed_qs.aggregate(
        teachers=Sum("teachers_attended"), leaders=Sum("leaders_attended")
    )
    ...
    clusters_covered = (
        reached_schools.values("cluster_id")
        .exclude(Q(cluster_id__isnull=True) | Q(cluster_id=""))
        .distinct()
        .count()
    )
    ...
    trainings_completed = verified_completed_qs.filter(
        activity_type__in=TRAINING_TYPES
    ).count()
    cluster_meetings_completed = verified_completed_qs.filter(
        activity_type="cluster_meeting"
    ).count()

    ssa_completed = SsaRecord.objects.filter(
        school__in=schools_in_lens, fy=fy, deleted_at__isnull=True
    ).count()
    ...
    records_prev = {
        r["school_id"]: r
        for r in SsaRecord.objects.filter(
            school__in=schools_in_lens, fy=prev_fy, deleted_at__isnull=True
        ).values("id", "school_id", "average_score")
    }
    ...
    curr_avgs = dict(
        SsaScore.objects.filter(ssa_record_id__in=curr_rec_ids)
        .values("intervention")
        .annotate(a=Avg("score"))
        .values_list("intervention", "a")
    )
    prev_avgs = dict(
        SsaScore.objects.filter(ssa_record_id__in=prev_rec_ids)
        .values("intervention")
        .annotate(a=Avg("score"))
        .values_list("intervention", "a")
    )
    ...
    pending_qs = Activity.objects.filter(
        deleted_at__isnull=True, fy=fy, status="evidence_uploaded"
    )
    if lens == "team" and scope.supervised_staff_ids:
    ...
    salesforce_ids_pending = completed_qs.filter(
...
def recruitment_recommendation(principal, query: dict) -> dict:
    ...
    if total == 0:
        return {
            ...
            "capacity": {
                ...
                "client": 0,
                "reachedPct": 0,
                "partnerPaymentBacklog": 0,
                "partnerEvidencePending": 0,
                "partnerStrainPct": 0,
            },
            "ssaReadiness": {
                "currentSsaPct": 0,
    ...
    if not scope.country_scope:
        if scope.staff_ids:
            completed_acts = completed_acts.filter(
    ...
    reached_schools = (
        completed_acts.filter(school__isnull=False)
        .values("school_id")
        .distinct()
        .count()
    )
    ...
    partner_backlog = Activity.objects.filter(
        deleted_at__isnull=True,
        fy=fy,
        delivery_type="partner",
        status="awaiting_ia_verification",
    ).count()
    partner_evidence_pending = Activity.objects.filter(
        deleted_at__isnull=True,
        fy=fy,
        delivery_type="partner",
        status="evidence_uploaded",
    ).count()
    partner_total = Activity.objects.filter(
        deleted_at__isnull=True, fy=fy, delivery_type="partner"
    ).count()
    partner_strain = (
        round((partner_evidence_pending / partner_total * 100), 1)
        if partner_total
        else 0.0
    )
    ...
    impact_ready_pct = (
        round(improved / total_compared * 100, 1) if total_compared > 0 else 0.0
    )

    missing_cluster = schools.filter(
        Q(cluster_id__isnull=True) | Q(cluster_id="")
    ).count()
    unmatched_owner = schools.filter(
        Q(account_owner_id__isnull=True) | Q(account_owner_id="")
    ).count()
    duplicates_count = schools.exclude(duplicate_status="none").count()
    missing_enrollment = schools.filter(
        Q(enrollment__isnull=True) | Q(enrollment__lte=0)
    ).count()

    total_gaps = (
        missing_cluster + unmatched_owner + duplicates_count + missing_enrollment
    )
    ...
    return {
        ...
        "impact": {"schoolsImproved": improved, "schoolsDeclined": declined},
...
def cluster_impact(cluster_id: str, principal) -> dict:
    ...
    improved_count = sum(
        1 for a in activities_out if a.get("impact", {}).get("status") == "Improved"
    )
    declined_count = sum(
        1 for a in activities_out if a.get("impact", {}).get("status") == "Declined"
    )
    no_change_count = sum(
        1 for a in activities_out if a.get("impact", {}).get("status") == "No Change"
    )
...
__all__ = [
    ...
    "coverage_summary",
    "geo_map_districts",
    "geo_map_district_detail",
    "school_directory_summary",
    "ssa_performance",
    "ssa_performance_grouped",
    "intervention_improvement",
    "support_ssa_correlation",
    "staff_vs_partner_correlation",
    "activity_pipeline",
    "contribution_summary",
    "recruitment_recommendation",
    "activity_impact_report",
    "school_impact",
    "cluster_impact",
]
