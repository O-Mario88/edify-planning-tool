...
class CDDashboardService:
    ...
    def get_dashboard(user, fy: str | None = None, month: int | None = None) -> dict:
        ...
        return {
            ...
            "leadership_attention": CDDashboardService.leadership_attention(cd, acts, fy, regional),
            ...
            "operational_risk": CDDashboardService.operational_risk_backlog(cd, acts, fy),
    ...
    def kpis(cd, acts, fy, pl_rows) -> list[dict]:
        ...
        active_schools = completed.exclude(school_id__isnull=True).values("school_id").distinct().count()
        ...
        sf_with = sf_required.exclude(Q(salesforce_activity_id__isnull=True) | Q(salesforce_activity_id="")).count()
        sf_compliance = _pct(sf_with, sf_total)

        pending = CDDashboardService._pending_cd_items(fy)
        high_risk_teams = sum(1 for r in pl_rows["rows"] if r["risk"] in HIGH_RISK_PL_BANDS)

        def card(icon, label, value, variant, helper, link=""):
            return {"icon": icon, "label": label, "value": value,
                    "variant": variant, "helper": helper, "link": link}

        return [
            card("target", "Country Target Progress", target_progress, "primary",
                 "valid completed vs assigned targets", "/analytics/country-director"),
            card("school", "Active Schools Served", f"{active_schools:,}", "success",
                 "completed activity this period"),
            card("shield", "Core Schools On Track", f"{core['pct']}%", "info",
                 f"{core['on_track']} of {core['total']} core plans", "/core-schools"),
            card("users", "Staff Productivity", f"{productivity}%", "info",
                 f"{completed.count():,} of {planned_n:,} activities completed"),
            card("cloud", "Activity SF ID Compliance", f"{sf_compliance}%", "violet",
                 f"{sf_with:,} of {sf_total:,} requiring SF IDs", "/completed-activities"),
            card("clock", "Pending Fund Requests", str(pending["count"]), "warning",
                 pending["amount_label"], "/fund-requests/weekly"),
            card("currency", "Budget Utilization",
                 analytics["Budget Utilization"]["value"], "finance",
                 "disbursed vs requested pipeline"),
            card("warning", "High-Risk Teams", str(high_risk_teams), "danger",
    ...
    def _core_on_track(fy) -> dict:
        ...
        total = plans.count()
        on_track = plans.filter(
            baseline_average__isnull=False
        ).filter(Q(visits_completed__gt=0) | Q(trainings_completed__gt=0)).count()
        return {"total": total, "on_track": on_track, "pct": _pct(on_track, total),
    ...
    def _pending_cd_items(fy) -> dict:
        ...
        budget_n = MonthlyWorkPlanBudget.objects.filter(
            fy=fy, status__in=["draft_generated", "cd_review", "admin_plan_added", "returned_by_rvp"]
        ).count()
        return {
            ...
            "budgets": budget_n,
            "amount": amount,
            "amount_label": (f"{_ugx_compact(amount)} pending" if amount else
                             ("monthly budget needs review" if budget_n else "nothing waiting")),
        }
    ...
    def leadership_attention(cd, acts, fy, regional) -> list[dict]:
        ...
        behind = [r for r in regional["rows"] if r["achievement"] < REGION_BEHIND_THRESHOLD]
        if behind:
            worst = behind[-1]
            cards.append({
                "tone": "danger", "title": f"{len(behind)} Region{'s' if len(behind) > 1 else ''} Behind Target",
        ...
        if overdue_sf:
            cards.append({
                "tone": "warning", "title": "High Activity SF ID Backlog",
        ...
        if pending["count"]:
            cards.append({
                "tone": "info", "title": f"{pending['count']} Fund Item{'s' if pending['count'] > 1 else ''} Pending",
                "line1": (f"{pending['amount_label']} awaiting Country Director action."
                          if pending["weekly"] else "Country monthly budget awaiting your review."),
                "action": "Review Approvals",
                "link": "/fund-requests/weekly" if pending["weekly"] else "/country-budget/",
            })
        return cards[:3]
    ...
    def country_performance(cd) -> dict:
        ...
        for m in range(1, 13):
            ...
            mq = full.filter(planned_date__gte=start.date(), planned_date__lt=end.date())
        ...
        return {"labels": base["labels"], "pct": base["pct"], "has_target": base["has_target"],
                "planned": planned, "completed": completed, "verified": verified}
    ...
    def regional_performance(cd, acts) -> dict:
        ...
        region_ids = list(
            schools.exclude(region__isnull=True).order_by("region_id")
            .values_list("region_id", flat=True).distinct()
        )
        names = dict(Region.objects.filter(id__in=region_ids).values_list("id", "name"))
        rows = []
        for rid in region_ids:
            r_school_ids = set(schools.filter(region_id=rid).values_list("id", flat=True))
            ...
            no_ssa = schools.filter(region_id=rid).exclude(current_fy_ssa_status="done").count()
            risk = ("danger" if achievement < 50 else
                    "warning" if achievement < REGION_BEHIND_THRESHOLD else "success")
            rows.append({
                "id": rid, "name": names.get(rid, "Region"),
                "achievement": achievement, "planned": planned, "completed": done,
                "schools": len(r_school_ids), "no_ssa": no_ssa, "tone": risk,
            })
    ...
    def pl_performance(cd, acts) -> dict:
        ...
        band = {"Low Risk": ("On Track", "success"), "Medium Risk": ("Watch", "amber"),
                "High Risk": ("High Risk", "warning"), "Critical Risk": ("Critical", "danger")}
        rows = []
        for pl in CDAnalyticsService._pls():
            ...
            team_acts = acts.filter(Q(responsible_staff_id__in=ids) | Q(school_id__in=school_ids))
            ...
            rows.append({
                "id": pl.id, "name": pl.name, "region": CDDashboardService._pl_region(school_ids),
                "target_pct": b["target_pct"], "staff": len(cceos),
                "planned": planned, "verified": verified,
                "sf_pending": sf_pending, "backlog": b["backlog"],
                "risk": label, "risk_tone": tone,
            })
    ...
    def _pl_region(school_ids) -> str:
        if not school_ids:
            return "—"
        r = (School.objects.filter(id__in=school_ids).exclude(region__isnull=True)
             .values_list("region__name", flat=True).first())
        return r or "—"
    ...
    def finance_snapshot(cd, acts, fy) -> dict:
        ...
        pending_rows = []
        qs = (WeeklyFundRequest.objects.filter(fy=fy, status="submitted_to_cd")
              .order_by("week_start_date")[:6])
        names = dict(User.objects.filter(
            id__in=[r.responsible_user for r in qs]).values_list("id", "name"))
        for r in qs:
            covered = AdvanceRequest.objects.filter(
                responsible_user_id=r.responsible_user,
                planned_date__gte=r.week_start_date, planned_date__lte=r.week_end_date,
            ).count()
            pending_rows.append({
                "id": r.id, "team": names.get(r.responsible_user, "Team"),
                "amount": _ugx_compact(int(r.total_amount or 0)),
                "covered": covered, "stage": "CD Review",
            })

        # Funded but not completed — money out, work not done.
        funded = AdvanceRequest.objects.filter(
            activity__fy=fy, activity__deleted_at__isnull=True,
            status__in=["disbursed", "accountability_pending", "accounted"],
        ).exclude(activity__status__in=COMPLETED_STATUSES).select_related("activity")
        today = date.today()
        funded_amount = int(funded.aggregate(s=Sum("disbursed_amount"))["s"] or 0)
        overdue = sum(1 for a in funded if a.activity and a.activity.planned_date and a.activity.planned_date < today)
        not_started = sum(1 for a in funded if a.activity and a.activity.status in ("scheduled", "planned"))
        return {
    ...
    def operational_risk_backlog(cd, acts, fy) -> list[dict]:
        ...
        visited = set(completed.filter(activity_type__in=VISIT_TYPES)
                      .exclude(school_id__isnull=True).values_list("school_id", flat=True))
        trained = set(completed.filter(activity_type__in=TRAINING_TYPES)
                      .exclude(school_id__isnull=True).values_list("school_id", flat=True))
        core = CDDashboardService._core_on_track(fy)
        leave_alerts = CDDashboardService._leave_conflicts(cd, acts)

        def c(icon, label, count, tone, helper, link):
            return {"icon": icon, "label": label, "count": count, "tone": tone,
                    "helper": helper, "link": link}

        return [
            c("cloud", "Overdue Activity SF IDs", overdue_sf, "danger",
              f"{SF_ID_OVERDUE_DAYS}+ days", "/completed-activities"),
            c("warning", "Returned Verifications", returned, "warning",
              "sent back by PL / IA", "/analytics/country-director"),
            c("school", "Schools No Recent Visit", len(all_ids - visited), "warning",
              "this period", "/analytics/country-director/drilldown?drill=risk&issue=no_visit"),
            c("book", "Schools No Training", len(all_ids - trained), "info",
              "this period", "/analytics/country-director/drilldown?drill=risk&issue=no_training"),
            c("shield", "Core Schools Behind", core["behind"], "violet",
              "assessment + 4 visits + 4 trainings", "/core-schools"),
            c("calendar", "Leave / Conflict Alerts", leave_alerts, "info",
              "coverage clashes next 7 days", "/leave/coverage"),
        ]
    ...
    def ssa_matrix(cd, acts) -> dict:
        ...
        def matrix_row(label, school_ids, kind, rid=""):
            rids = list(SsaRecord.objects.filter(
                school_id__in=school_ids, verification_status="confirmed", fy=latest
            ).values_list("id", flat=True))
            if not rids:
                return None
            by = {r["intervention"]: r["a"] for r in SsaScore.objects.filter(
                ssa_record_id__in=rids).values("intervention").annotate(a=Avg("score"))}
            cells = []
            for v, _label, _code in SSA_INTERVENTIONS:
                pct = _norm(by.get(v))
                cells.append({"pct": pct, "tone": ssa_band(pct)[2]})
            overall = _norm(SsaRecord.objects.filter(id__in=rids).aggregate(a=Avg("average_score"))["a"])
            return {"label": label, "kind": kind, "id": rid, "cells": cells,
                    "overall": overall, "overall_tone": ssa_band(overall)[2]}

        schools = School.objects.filter(id__in=cd.school_ids)
        rows = []
        for rid in schools.exclude(region__isnull=True).order_by("region_id").values_list("region_id", flat=True).distinct():
            name = Region.objects.filter(id=rid).values_list("name", flat=True).first() or "Region"
            row = matrix_row(name, set(schools.filter(region_id=rid).values_list("id", flat=True)), "region", rid)
            if row:
                rows.append(row)
    ...
    def priority_schools(cd, acts, limit=6) -> list[dict]:
        ...
        completed = acts.filter(status__in=COMPLETED_STATUSES)
        schools = School.objects.filter(id__in=cd.school_ids).select_related("district", "region")
        visited = set(completed.filter(activity_type__in=VISIT_TYPES)
                      .exclude(school_id__isnull=True).values_list("school_id", flat=True))
        trained = set(completed.filter(activity_type__in=TRAINING_TYPES)
                      .exclude(school_id__isnull=True).values_list("school_id", flat=True))
        ...
        weak = set()
        if latest:
            weak = set(SsaRecord.objects.filter(
                school_id__in=cd.school_ids, verification_status="confirmed",
                fy=latest, average_score__lt=5.0,
            ).values_list("school_id", flat=True))
        sf_missing = set(_requires_sf_id(acts).filter(
            Q(salesforce_activity_id__isnull=True) | Q(salesforce_activity_id="")
        ).exclude(school_id__isnull=True).values_list("school_id", flat=True))
        ...
        for s in schools:
            ...
            if len(issues) >= 2:
                ...
                rows.append({
                    ...
                    "region": (s.district.name if s.district_id else
                               (s.region.name if s.region_id else "—")),
                    "issues": issues[:3], "risk": risk,
    ...
    def quick_actions() -> list[dict]:
        return [
            {"label": "Review Fund Requests", "icon": "currency", "url": "/fund-requests/weekly",
             "helper": "CD approval queue"},
            {"label": "View High-Risk Regions", "icon": "warning", "url": "/analytics/country-director",
             "helper": "regional risk intelligence"},
            {"label": "Inspect Activity SF ID Backlog", "icon": "cloud", "url": "/completed-activities",
             "helper": "completed work missing SF IDs"},
            {"label": "See Core School Delays", "icon": "shield", "url": "/core-schools",
             "helper": "assessment + 4 visits + 4 trainings"},
            {"label": "Review Team Targets", "icon": "target", "url": "/team-targets/",
             "helper": "PL and CCEO target health"},
            {"label": "Open Country Report", "icon": "report", "url": "/reports",
    ...
    def budget_stage(fy) -> dict:
        ...
        if not b:
            return {"label": "Not Started", "helper": "no monthly budget yet",
                    "status": None, "month": None, "amount": None}
        label, helper = labels.get(b.status, (b.status, ""))
        return {"label": label, "helper": helper, "status": b.status,
                "month": b.month_key, "amount": _ugx_compact(int(b.total_amount or 0))}
