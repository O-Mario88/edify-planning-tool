...
INTERVENTIONS = [
    "christlike_behaviour", "exposure_to_word_of_god", "financial_health",
    "leadership", "learning_environment", "government_requirement",
...
class PLAnalyticsTest(TestCase):
    def setUp(self):
        ...
        self.pl_a, self.pl_a_sp = self._staff("pla@t.org", "PL Alpha", EdifyRole.COUNTRY_PROGRAM_LEAD.value)
        self.cceo_a1, self.cceo_a1_sp = self._staff("a1@t.org", "Ann A1", EdifyRole.CCEO.value)
        StaffSupervisorAssignment.objects.create(supervisor=self.pl_a_sp, supervisee=self.cceo_a1_sp)

        self.pl_b, self.pl_b_sp = self._staff("plb@t.org", "PL Beta", EdifyRole.COUNTRY_PROGRAM_LEAD.value)
        self.cceo_b1, self.cceo_b1_sp = self._staff("b1@t.org", "Ben B1", EdifyRole.CCEO.value)
        StaffSupervisorAssignment.objects.create(supervisor=self.pl_b_sp, supervisee=self.cceo_b1_sp)
        ...
        School.objects.filter(id__in=[self.sch_a1.id, self.sch_a2.id, self.sch_a3.id]).update(
            cluster_id="cl-a", cluster_status="clustered"
        )

        StaffSchoolAssignment.objects.create(staff=self.cceo_a1_sp, school_id=self.sch_a1.id)
        StaffSchoolAssignment.objects.create(staff=self.cceo_a1_sp, school_id=self.sch_a2.id)
        StaffSchoolAssignment.objects.create(staff=self.cceo_a1_sp, school_id=self.sch_a3.id)
        StaffSchoolAssignment.objects.create(staff=self.cceo_b1_sp, school_id=self.sch_b1.id)
        ...
        self._activity(self.cceo_a1_sp.id, self.sch_a1, "training", "staff", teachers=10, leaders=2)
        self.partner = Partner.objects.create(name="Partner P")
        self._activity(self.cceo_a1_sp.id, self.sch_a1, "partner_activity", "partner", partner_id=self.partner.id)
        PartnerAssignment.objects.create(cluster=self.cluster_a, partner=self.partner, assigning_staff_id=self.cceo_a1_sp.id)
        ...
        StaffTargetProfile.objects.create(
            staff=self.cceo_a1_sp, fy=FY, visits_target=2, trainings_target=1,
            cluster_meetings_target=1, ssa_target=1,
        )
    ...
    def _school(self, sid, district, stype="client", ssa_done=True):
        return School.objects.create(
            school_id=f"SCH-{sid}", name=f"School {sid}", region=self.region, district=district,
            school_type=stype, enrollment=100,
    ...
    def _ssa(self, school, fy, avg, status="confirmed"):
        rec = SsaRecord.objects.create(
            school=school, date_of_ssa=timezone.now(), fy=fy, quarter="Q2",
            average_score=avg, verification_status=status,
        )
        for i in INTERVENTIONS:
            SsaScore.objects.create(ssa_record=rec, intervention=i, score=avg)
        return rec

    def _activity(self, responsible_sp_id, school, atype, delivery, teachers=0, leaders=0, partner_id=None):
        return Activity.objects.create(
            school=school, activity_type=atype, delivery_type=delivery, status="completed",
            responsible_staff_id=responsible_sp_id, assigned_partner_id=partner_id,
            fy=FY, quarter="Q3", planned_date=date(2026, 4, 10),
            scheduled_date=timezone.make_aware(timezone.datetime(2026, 4, 10, 9, 0)),
            evidence_status="accepted", teachers_attended=teachers, leaders_attended=leaders,
        )
    ...
    def test_pl_insight_creates_todo_when_threshold_met(self):
        ...
        School.objects.filter(id__in=[self.sch_a1.id, self.sch_a2.id, self.sch_a3.id]).update(current_fy_ssa_status="done")
