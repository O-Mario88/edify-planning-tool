...
class RVPDashboardService:
    ...
    def get_dashboard(user, fy: str | None = None, month: str | None = None) -> dict:
        ...
        # teams see (this rollup calls _weighted_overall repeatedly below;
        # one refresh here covers all of them).
        _refresh_target_ledger(cd)
        completed_qs = acts.filter(status__in=COMPLETED_STATUSES)
        verified_qs = acts.filter(status__in=VERIFIED_STATUSES)
        ...
        high_risk_regions = [r for r in regions if r["status"] in ("High Risk", "Critical")]
        ...
        projects_on_track = sum(
            1 for p in projects if p["impact"] in ("Great Impact", "Positive Impact")
            and p["target_pct"] >= 60)
        under_review = [p for p in projects
                        if p["recommendation"] in ("Request Redesign", "Put Under Review",
                                                    "Close Project")]
        ...
        reached = set(completed_qs.exclude(school_id__isnull=True)
                      .values_list("school_id", flat=True))
        teachers = int(completed_qs.filter(activity_type__in=TRAINING_TYPES)
                       .aggregate(s=Sum("teachers_attended"))["s"] or 0)
        ...
        kpis = [
            card("target", "Regional Target Achievement", f"{overall_pct}%", "primary",
                 "weighted five-area validated ledger"),
            card("map", "Regions Overseen", str(len(regions)), "info",
                 "with active schools"),
            card("users", "Country Directors On Track", f"{on_track_cds} / {len(cds)}",
                 "success" if on_track_cds == len(cds) and cds else "warning",
                 "performance + finance thresholds"),
            card("clock", "Monthly Budget Pending Approval",
            ...
            card("report", "Annual Budget Status",
            ...
            card("shield", "Special Projects On Track",
                 f"{projects_on_track} / {len(projects)}", "violet",
                 "execution + verified annual impact"),
            card("currency", "Budget Utilization", f"{budget_util}%", "finance",
                 "disbursed vs approved"),
            card("school", "Schools Impacted", f"{len(reached):,}", "info",
                 "verified activity"),
            card("users", "Teachers Trained", f"{teachers:,}", "success",
                 "verified attendance"),
            card("warning", "High-Risk Regions", str(len(high_risk_regions)),
        ...
        if awaiting.count():
            ...
            age = ((timezone.now() - oldest.submitted_at).days
                   if oldest and oldest.submitted_at else 0)
            attention.append({
                "key": "budgets", "tone": "warning",
                "title": "Country Budget Pending Approval",
                "body": (f"{_ugx_compact(awaiting_amt)} across {awaiting.count()} "
                         f"monthly budget(s) awaiting your approval"
                         + (f" · oldest {age}d" if age else "")),
                "action": "Review Monthly Budgets", "drill": "approvals"})
        if high_risk_regions:
            attention.append({
                "key": "risk", "tone": "danger",
                "title": "High-Risk Regional Performance",
                "body": (f"{len(high_risk_regions)} region(s) below the risk "
                         "threshold: " + ", ".join(r["name"] for r in high_risk_regions[:3])),
                "action": "Inspect High-Risk Regions", "drill": "regions"})
        if under_review:
            attention.append({
                "key": "projects", "tone": "warning",
                "title": "Special Project Under Review",
                "body": f"{len(under_review)} project(s) need your review and decision.",
                "action": "Review Special Projects", "drill": "projects"})
        if not annuals.exists():
            attention.append({
                "key": "annual", "tone": "info",
                "title": "Annual Budget Not Submitted",
                "body": f"No FY {fy} Country Annual Budget has been prepared yet.",
                "action": "Review Annual Budgets", "drill": "approvals"})
        attention = attention[:3]

        # ── recent decisions + strategy notes ────────────────────────────────
        decisions = [{
            "label": d.subject_label, "action": d.action.title(),
            "amount": _ugx_compact(d.amount) if d.amount else "—",
            "by": "RVP", "when": d.created_at,
        } for d in RVPApprovalDecision.objects.order_by("-created_at")[:5]]

        notes = list(StrategyNote.objects.exclude(status="cancelled")
        ...
        return {
            ...
            "annual_rows": [RVPDashboardService._annual_row(b) for b in
                            annuals.order_by("-fy")],
            "projects": projects,
            "project_impact_dist": RVPDashboardService._impact_dist(projects),
            "partners": RVPDashboardService.partner_growth(cd, acts),
            "allocation": RVPDashboardService.resource_allocation(cd, acts, fy, reached),
            "risks": RVPDashboardService.operational_risks(cd, acts, awaiting, projects),
            ...
            "quick_actions": [
                {"label": "Review Monthly Budgets", "url": "/country-budget/", "icon": "report"},
                {"label": "Review Annual Country Budgets", "url": "#rvp-approvals", "icon": "calendar"},
                {"label": "Inspect High-Risk Regions", "url": "#region-ranking", "icon": "warning"},
                {"label": "View Special Projects", "url": "#special-projects", "icon": "shield"},
                {"label": "Review Partner Performance", "url": "#partner-growth", "icon": "users"},
    ...
    def performance_overview(cd, acts, fy):
        from apps.targets.fy_calendar import MONTH_LABELS, FinancialYearCalendarService as Cal
        ...
        validated_by_month = {
            ...
            ).values("credited_month").annotate(n=Count("id"))
        }
        ...
        return {"labels": MONTH_LABELS, "planned": planned, "completed": completed,
                "verified": verified, "achievement": achievement}
    ...
    def region_ranking(cd, acts, fy):
        ...
        for region in Region.objects.all():
            sids = [sid for sid, rname in region_map.items() if rname == region.name]
            if not sids:
                continue
            r_acts = acts.filter(school_id__in=sids)
            planned = r_acts.count()
            done = r_acts.filter(status__in=COMPLETED_STATUSES).count()
            ver = r_acts.filter(status__in=VERIFIED_STATUSES).count()
            ...
            if latest:
                ...
                ssa = _norm(SsaRecord.objects.filter(
                    school_id__in=sids, verification_status="confirmed", fy=latest
                ).aggregate(a=Avg("average_score"))["a"])
            sf_have = r_acts.filter(status__in=COMPLETED_STATUSES).exclude(
                salesforce_activity_id__isnull=True).exclude(
                salesforce_activity_id="").count()
            sf_rate = _pct(sf_have, done) if done else 100
            score = round(
                exec_rate * 0.35 + verified_rate * 0.2 + (ssa or 0) * 0.25
                + sf_rate * 0.2)
            if score >= 80:
            ...
            rows.append({
                "name": region.name, "id": region.id, "schools": len(sids),
                "exec_rate": exec_rate, "verified_rate": verified_rate,
                "ssa": ssa, "sf_rate": sf_rate, "score": score,
    ...
    def cd_performance(cd, acts, fy):
        ...
        sf_have = done_qs.exclude(salesforce_activity_id__isnull=True).exclude(
            salesforce_activity_id="").count()
        backlog = (done - sf_have) + acts.filter(status="awaiting_ia_verification").count()
        util = CDAnalyticsService._budget_utilization(cd)
        pending_cd = WeeklyFundRequest.objects.filter(status="submitted_to_cd").count()

        rows = []
        for u in User.objects.filter(roles__contains=["CountryDirector"],
                                     status="active", deleted_at__isnull=True):
            score = round(overall_pct * 0.4 + _pct(ver, done) * 0.2
                          + _pct(sf_have, done) * 0.2 + min(util, 100) * 0.2)
            if score >= 75:
    ...
    def special_projects(cd, acts, fy):
        ...
        for p in Project.objects.filter(deleted_at__isnull=True):
            p_acts = acts.filter(Q(project_id=p.id) | Q(schedule_cost_lines__project=p)).distinct()
            planned = p_acts.count()
            done = p_acts.filter(status__in=COMPLETED_STATUSES).count()
            ver = p_acts.filter(status__in=VERIFIED_STATUSES).count()
            budget = int(p_acts.aggregate(s=Sum("schedule_cost_lines__amount"))["s"] or 0)
            school_ids = set(p.school_links.values_list("school_id", flat=True)) \
                if hasattr(p, "school_links") else set(
                    p_acts.exclude(school_id__isnull=True).values_list("school_id", flat=True))
            ...
            if latest and prev and school_ids:
                cur = SsaRecord.objects.filter(
                    school_id__in=school_ids, verification_status="confirmed",
                    fy=latest).aggregate(a=Avg("average_score"))["a"]
                old = SsaRecord.objects.filter(
                    school_id__in=school_ids, verification_status="confirmed",
                    fy=prev).aggregate(a=Avg("average_score"))["a"]
            ...
            if not planned and not school_ids:
                ...
                rec = "Request Redesign" if impact != "Negative Impact" else "Put Under Review"
            elif not high_exec and high_impact:
                rec = "Continue Project"
            else:
                rec = "Close Project" if impact == "Negative Impact" else "Put Under Review"
            rows.append({
                "id": p.id, "name": p.name, "code": p.code or "—",
                "country": "Uganda",
                "goal": p.get_category_display(),
                "intervention": p.intervention or "—",
                "budget": _ugx_compact(budget), "budget_raw": budget,
                "target_pct": target_pct, "verified": ver,
                "impact": impact, "impact_tone": impact_tone, "delta": delta,
                "recommendation": rec,
            })
    ...
    def _impact_dist(projects):
        keys = ["Great Impact", "Positive Impact", "No Measurable Impact",
                "Negative Impact", "Impact Not Measurable Yet"]
        return [{"label": k, "count": sum(1 for p in projects if p["impact"] == k)}
                for k in keys if any(p["impact"] == k for p in projects)]
    ...
    def partner_growth(cd, acts):
        ...
        for r in base:
            out.append({
                **r,
                "type": "Implementing Partner" if r.get("schools_supported") else
                        "Strategic Partner",
                "strategic_value": ("Strong" if (r.get("ssa_improve") or 0) > 5
                                    else "Moderate" if (r.get("ssa_improve") or 0) > 0
                                    else "Watch"),
            })
        return out
    ...
    def resource_allocation(cd, acts, fy, reached):
        ...
        lines = (ActivityScheduleCostLine.objects.filter(
            ...
            .values("activity__school_id").annotate(s=Sum("amount")))
        for row in lines:
        ...
        for rname, amt in sorted(by_region.items(), key=lambda kv: -kv[1]):
            ...
            funding.append({"region": rname, "amt": _ugx_compact(amt),
                            "share": share, "flag": flag})
        month_start = date.today().replace(day=1)
        new_schools = acts.filter(
            status__in=COMPLETED_STATUSES, planned_date__gte=month_start
        ).exclude(school_id__isnull=True).values("school_id").distinct().count()
        learners = int(School.objects.filter(id__in=reached)
                       .aggregate(s=Sum("enrollment"))["s"] or 0)
        cost_per_school = _ugx_compact(round(total / max(1, len(reached))))
        return {"funding": funding, "schools_reached": len(reached),
                "new_schools_month": new_schools, "learners": learners,
    ...
    def operational_risks(cd, acts, awaiting, projects):
        cards = [c for c in CDAnalyticsService.operational_risk(cd, acts)
                 if c["count"]]
        stale = [b for b in awaiting
                 if b.submitted_at and (timezone.now() - b.submitted_at).days > 7]
        if stale:
            cards.append({"label": "Country Budget Approval Delayed",
                          "count": len(stale), "tone": "danger",
                          "aging": "7+ days", "link": "#rvp-approvals"})
        weak = sum(1 for p in projects
                   if p["impact"] in ("Negative Impact", "No Measurable Impact"))
        if weak:
            cards.append({"label": "Projects Underperforming", "count": weak,
                          "tone": "warning", "aging": "", "link": "#special-projects"})
        return cards[:8]

    # ── §26 recommended next actions ─────────────────────────────────────────
    @staticmethod
    def recommended_actions(awaiting, annual_pending, high_risk_regions,
                            under_review, notes):
        out = []
        if awaiting.count():
            out.append({"label": f"Approve {awaiting.count()} pending monthly "
                                 f"budget{'s' if awaiting.count() != 1 else ''}",
                        "priority": "high", "url": "#rvp-approvals"})
        if annual_pending.count():
            out.append({"label": f"Review {annual_pending.count()} annual budget "
                                 "submission(s)", "priority": "high",
                        "url": "#rvp-approvals"})
        if high_risk_regions:
            out.append({"label": f"Review {len(high_risk_regions)} high-risk "
                                 f"region{'s' if len(high_risk_regions) != 1 else ''}",
                        "priority": "high", "url": "#region-ranking"})
        if under_review:
            out.append({"label": f"Evaluate {len(under_review)} special "
                                 f"project{'s' if len(under_review) != 1 else ''}",
                        "priority": "medium", "url": "#special-projects"})
        open_notes = [n for n in notes if n.status == "open"]
        if open_notes:
            out.append({"label": f"Follow up {len(open_notes)} open strategy "
                                 f"note{'s' if len(open_notes) != 1 else ''}",
                        "priority": "medium", "url": "#strategy-notes"})
        return out[:6]
    ...
    def _budget_row(b):
        return {
            ...
            "status": b.get_status_display(), "status_key": b.status,
