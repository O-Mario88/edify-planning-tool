...
class RoleDashboardsTest(TestCase):
    def setUp(self):
        ...
        self.rvp, _ = _staff("rvp@t.org", "RVP", EdifyRole.REGIONAL_VICE_PRESIDENT.value)
        self.hr, self.hr_sp = _staff("hr@t.org", "HR", EdifyRole.HUMAN_RESOURCES.value)
        self.pl, self.pl_sp = _staff("pl@t.org", "PL One", EdifyRole.COUNTRY_PROGRAM_LEAD.value)
    ...
    def test_cd_dashboard_surfaces_escalated_requests_and_approves(self):
        wfr = WeeklyFundRequest.objects.create(
            ...
            responsible_user=self.pl.id, total_amount=250_000, status="submitted_to_cd",
        )
        ...
        self.assertEqual(d2["finance_snapshot"]["pending_count"], 0)  # queue auto-clears

    def test_cd_dashboard_budget_stage_reads_monthly_budget(self):
        MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key="2026-08", status="submitted_to_rvp", total_amount=5_000_000,
        )
        d = CDDashboardService.get_dashboard(self.cd, fy=FY)
    ...
    def test_hr_dashboard_counts_real_leave_workflow(self):
        Leave.objects.create(
            staff=self.cceo_sp, type="annual", status="pending", days=3,
            start_date=(date.today() + timedelta(days=3)).isoformat(),
            end_date=(date.today() + timedelta(days=5)).isoformat(),
        )
        Leave.objects.create(
            staff=self.pl_sp, type="annual", status="approved", days=1,
            start_date=date.today().isoformat(), end_date=date.today().isoformat(),
        )
        PublicHoliday.objects.create(name="Independence Day",
    ...
    def test_hr_coverage_clash_requires_scheduled_work(self):
        ...
        district = District.objects.create(name="D", region=region, district_type="primary")
        school = School.objects.create(school_id="S-1", name="S1", region=region, district=district)
        lv_start = date.today() + timedelta(days=2)
        Leave.objects.create(
            staff=self.cceo_sp, type="annual", status="approved", days=2,
            start_date=lv_start.isoformat(), end_date=(lv_start + timedelta(days=1)).isoformat(),
        )
        d = HRDashboardService.get_dashboard(self.hr)
        self.assertEqual(
            {k["label"]: k["value"] for k in d["kpi_strip_items"]}["Coverage Clashes (7d)"], "0"
        )  # leave alone is not a clash
        Activity.objects.create(
            school=school, activity_type="school_visit", delivery_type="staff",
            status="scheduled", responsible_staff_id=self.cceo_sp.id, fy=FY, quarter="Q4",
            planned_date=lv_start,
            scheduled_date=tz.make_aware(tz.datetime(lv_start.year, lv_start.month, lv_start.day, 9)),
        )
        d2 = HRDashboardService.get_dashboard(self.hr)
        self.assertEqual(
            {k["label"]: k["value"] for k in d2["kpi_strip_items"]}["Coverage Clashes (7d)"], "1"
        )

    # ── RVP dashboard ────────────────────────────────────────────────────────
    def test_rvp_dashboard_queues_submitted_budgets(self):
        MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key="2026-08", status="submitted_to_rvp",
            program_total=4_000_000, admin_total=1_000_000, total_amount=5_000_000,
            activity_count=40,
        )
        MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key="2026-07", status="approved_by_rvp", total_amount=3_000_000,
        )
    ...
    def test_rvp_dashboard_rebuilds_stale_target_ledger(self):
        ...
        district = District.objects.create(name="D-RVP", region=region, district_type="primary")
        school = School.objects.create(
            school_id="S-RVP", name="RVP School", region=region, district=district,
        ...
        Activity.objects.create(
            school=school, activity_type="school_visit", delivery_type="staff", status="completed",
            responsible_staff_id=self.cceo_sp.id, fy=FY, quarter="Q3",
