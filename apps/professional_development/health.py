...
def pd_health_checks() -> dict:
    ...
    closed = ProfessionalDevelopmentRequest.objects.filter(status=PDStatus.COMPLETED_CLOSED)

    # 1. Closed without an uploaded certificate.
    closed_no_cert = [
        f"{r.staff_name} — {r.course_name}" for r in closed
    ...
    closed_funded_no_netsuite = [
        f"{r.staff_name} — {r.course_name}" for r in
        closed.filter(funding_type__in=FUNDED_TYPES)
        if not (r.accountability_netsuite_id or "").strip()
    ]

    # 3. Self-signoff leak — signed off by the record's own owner.
    self_signoff = [
        f"{r.staff_name} — {r.course_name}" for r in closed.exclude(signed_off_by__isnull=True)
        if r.signed_off_by == r.owner_user_id
    ]

    # 4. Self-approval leak — supervisor stage approved by the owner.
    supervisor_reviewed = ProfessionalDevelopmentRequest.objects.exclude(supervisor_reviewed_by__isnull=True)
    self_supervisor_approval = [
        f"{r.staff_name} — {r.course_name}" for r in supervisor_reviewed
        if r.supervisor_reviewed_by == r.owner_user_id
    ]

    # 5. Self-approval leak — HR stage approved by the owner.
    hr_reviewed = ProfessionalDevelopmentRequest.objects.exclude(hr_reviewed_by__isnull=True)
    self_hr_approval = [
        f"{r.staff_name} — {r.course_name}" for r in hr_reviewed
    ...
    self_disbursement = [
        ...
        for d in ProfessionalDevelopmentDisbursement.objects.select_related("fund_request__request")
    ...
    approved_or_later = ProfessionalDevelopmentRequest.objects.filter(
        ...
        status__in=(
            ...
            PDStatus.ACCOUNTABILITY_SUBMITTED, PDStatus.AWAITING_HR_SIGNOFF, PDStatus.COMPLETED_CLOSED,
        ),
    ).exclude(id__in=ProfessionalDevelopmentFundRequest.objects.values_list("request_id", flat=True))
    missing_fund_request = [f"{r.staff_name} — {r.course_name}" for r in approved_or_later]

    # 8. In-person/hybrid, submitted-or-later, with zero uploaded evidence.
    submitted_or_later_statuses = [s for s in PDStatus.values if s not in (PDStatus.DRAFT,)]
    ...
    missing_evidence = [
        f"{r.staff_name} — {r.course_name}" for r in evidence_required
    ...
    stale_bamboohr = [
        ...
        for r in ProfessionalDevelopmentRequest.objects.filter(status=PDStatus.CERTIFICATE_UPLOADED)
    ...
    stale_signoff = [
        f"{r.staff_name} — {r.course_name} ({(now - r.updated_at).days}d)"
        for r in ProfessionalDevelopmentRequest.objects.filter(status=PDStatus.AWAITING_HR_SIGNOFF)
        if (now - r.updated_at).days > 14
    ]

    # 13. Calendar blocks left behind by a request that is no longer active.
    inactive_with_block = ProfessionalDevelopmentRequest.objects.filter(
        status__in=(PDStatus.REJECTED, PDStatus.CANCELLED, PDStatus.WITHDRAWN)
    ).exclude(calendar_block_id__isnull=True).exclude(calendar_block_id="")
    ...
    negative_allocations = [
        ...
        for a in ProfessionalDevelopmentAllocation.objects.filter(annual_allocation__lt=0)
    ]

    return {
        "checks": [
            _check("pd_closed_no_certificate", "Closed PD records missing a certificate",
                   closed_no_cert, "blocking",
                   "A course cannot be signed off without an uploaded certificate — investigate how this closed."),
            _check("pd_closed_funded_no_netsuite", "Closed funded PD records missing a NetSuite Expense ID",
                   closed_funded_no_netsuite, "blocking",
                   "Non-negotiable: enter the NetSuite Expense ID before any funded course can close."),
            _check("pd_self_signoff", "PD records signed off by their own owner",
                   self_signoff, "blocking",
                   "HR must never sign off their own Professional Development record — reassign to an independent reviewer."),
            _check("pd_self_supervisor_approval", "PD records approved by their own supervisor stage",
                   self_supervisor_approval, "blocking",
                   "No manager may approve their own request — check the routing assignment."),
            _check("pd_self_hr_approval", "PD records approved by HR for themselves",
                   self_hr_approval, "blocking",
                   "HR must never approve its own staff's request — route to an independent HR/leadership reviewer."),
            _check("pd_self_disbursement", "PD funds disbursed by the requester's own account",
                   self_disbursement, "blocking",
                   "An Accountant must never disburse their own Professional Development funding."),
            _check("pd_missing_fund_request", "Funded PD requests with no dedicated fund request",
                   missing_fund_request, "blocking",
                   "Every approved funded course must have a PD fund request — funding must never bypass the dedicated PD ledger."),
            _check("pd_missing_evidence", "In-person/hybrid PD requests missing enrollment evidence",
                   missing_evidence, "blocking",
                   "In-person and hybrid courses require an uploaded admission/enrollment letter before submission."),
            _check("pd_stale_ended", "Courses ended over 30 days without being marked complete",
                   stale_ended, "warning",
                   "Follow up with the employee to mark the course complete and start the closure sequence."),
            _check("pd_stale_bamboohr", "Certificates uploaded over 14 days without BambooHR confirmation",
                   stale_bamboohr, "warning",
                   "Remind the employee to confirm the BambooHR upload."),
            _check("pd_stale_accountability", "Funded courses awaiting accountability over 14 days",
                   stale_accountability, "warning",
                   "Remind the employee to submit accountability and the NetSuite Expense ID."),
            _check("pd_stale_signoff", "Records awaiting HR sign-off over 14 days",
                   stale_signoff, "warning",
                   "HR should clear the sign-off queue — check the HR Sign-Off Queue."),
            _check("pd_orphaned_calendar_blocks", "Calendar blocks left behind by an inactive PD request",
                   orphaned_blocks, "warning",
                   "Clear the PD calendar block for rejected/cancelled/withdrawn requests."),
            _check("pd_stale_drafts", "Draft PD requests untouched over 90 days",
                   stale_drafts, "warning",
                   "Prompt the employee to submit or cancel the abandoned draft."),
            _check("pd_negative_allocation", "PD allocations with a negative annual amount",
                   negative_allocations, "blocking",
