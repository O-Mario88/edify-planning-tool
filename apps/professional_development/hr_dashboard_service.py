...
NOT_STARTED_STATUSES = (
    ...
    PDStatus.APPROVED_PENDING_FUNDING, PDStatus.APPROVED_UNFUNDED, PDStatus.DISBURSED,
...
INACTIVE_STATUSES = (
    PDStatus.RETURNED_BY_SUPERVISOR, PDStatus.RETURNED_BY_HR, PDStatus.REJECTED,
    PDStatus.CANCELLED, PDStatus.DEFERRED, PDStatus.WITHDRAWN,
)
PENDING_ACCOUNTABILITY_STATUSES = (PDStatus.BAMBOOHR_CONFIRMED, PDStatus.ACCOUNTABILITY_SUBMITTED)

STATUS_BUCKET_OPTIONS = [
    ("not_started", "Not Started"), ("in_progress", "In Progress"),
    ("pending_certificate", "Pending Certificate"), ("completed", "Completed"),
    ("signed_off", "Signed Off"), ("inactive", "Inactive / Returned"),
]
...
def _scoped_staff_ids(principal):
    ...
    if role == "CountryDirector":
        ...
        ids = list(StaffProfile.objects.filter(country=country).values_list("id", flat=True)) if country else []
        return ids, country
...
class HRPDDashboardService:
    ...
    def get_dashboard(principal, params: dict) -> dict:
        ...
        country_options = sorted(
            c for c in country_scope_qs.values_list("country", flat=True).distinct() if c
        )

        alloc_qs = ProfessionalDevelopmentAllocation.objects.filter(fy=fy)
        req_qs = ProfessionalDevelopmentRequest.objects.filter(fy=fy).exclude(status=PDStatus.DRAFT)
        if scoped_ids is not None:
        ...
        if role_filter:
            ...
            role_staff_ids = list(
                StaffProfile.objects.filter(user__active_role=role_filter).values_list("id", flat=True)
            )
            alloc_qs = alloc_qs.filter(staff_id__in=role_staff_ids)
            req_qs = req_qs.filter(staff_id__in=role_staff_ids)
        if search:
            req_qs = req_qs.filter(Q(staff_name__icontains=search) | Q(course_name__icontains=search))
        ...
        committed = sum(r.requested_amount_cents for r in all_rows if r.status in COMMITTED_STATUSES)
        accounted = sum(r.accounted_amount or 0 for r in all_rows)
        staff_enrolled = len({r.staff_id for r in all_rows})
        courses_in_progress = sum(1 for r in all_rows if r.status in IN_PROGRESS_STATUSES)
        pending_certificate = sum(1 for r in all_rows if r.status in PENDING_CERT_STATUSES)
        pending_accountability = sum(
            1 for r in all_rows
            if r.status in PENDING_ACCOUNTABILITY_STATUSES and r.funding_type in FUNDED_TYPES
        )
        signoff_pending = sum(1 for r in all_rows if r.status == PDStatus.AWAITING_HR_SIGNOFF)
        currency = alloc_qs.first().currency if alloc_qs.exists() else "UGX"

        kpis = [
            {"key": "allocation", "label": "Total Annual PD Allocation", "icon": "currency", "variant": "primary",
             "value": f"{currency} {total_allocation/100:,.0f}", "helper": f"FY {fy}"},
            {"key": "committed", "label": "Funds Committed", "icon": "chart", "variant": "default",
             "value": f"{currency} {committed/100:,.0f}",
             "helper": f"{round(committed/total_allocation*100) if total_allocation else 0}% of allocation"},
            {"key": "accounted", "label": "Funds Accounted For", "icon": "accountability", "variant": "default",
             "value": f"{currency} {accounted/100:,.0f}",
             "helper": f"{round(accounted/total_allocation*100) if total_allocation else 0}% of allocation"},
            {"key": "enrolled", "label": "Staff Enrolled", "icon": "users", "variant": "primary",
             "value": str(staff_enrolled), "helper": f"FY {fy}"},
            {"key": "in_progress", "label": "Courses In Progress", "icon": "graduation", "variant": "primary",
             "value": str(courses_in_progress), "helper": "Active now"},
            {"key": "pending_cert", "label": "Pending Certificate Uploads", "icon": "certificate",
             "variant": "warning" if pending_certificate else "success", "value": str(pending_certificate),
             "helper": "Course ended, no certificate yet"},
            {"key": "pending_acct", "label": "Pending Accountability", "icon": "expense",
             "variant": "warning" if pending_accountability else "success", "value": str(pending_accountability),
             "helper": "Funded, not yet cleared"},
            {"key": "signoff", "label": "HR Sign-Off Pending", "icon": "signoff",
             "variant": "warning" if signoff_pending else "success", "value": str(signoff_pending),
        ...
        def _reminder_tag(r):
            if r.status in PENDING_CERT_STATUSES:
                ...
                if overdue >= 7:
                    return "overdue"
                return "due"
            if r.status == PDStatus.ENROLLMENT_CONFIRMED and 0 <= (r.start_date - today).days <= 7:
                return "due"
            if r.status in PENDING_ACCOUNTABILITY_STATUSES and r.funding_type in FUNDED_TYPES:
        ...
        for r in all_rows:
            ...
            if reminder_filter and tag != reminder_filter:
                continue
            tracker_rows.append({
                "id": r.id, "staff_name": r.staff_name, "role": r.position or "—",
                "supervisor": r.supervisor_name or "—", "course_name": r.course_name,
                ...
                "status": r.get_status_display(), "bucket": bucket,
                "certificate_status": "Uploaded" if r.status not in NOT_STARTED_STATUSES + IN_PROGRESS_STATUSES + PENDING_CERT_STATUSES else "Pending",
                "accountability_status": (
                    "N/A" if r.funding_type not in FUNDED_TYPES
                    else "Cleared" if r.accountability_reviewed_at
                    else "Submitted" if r.accountability_submitted_at
                    else "Pending"
                ),
                "signoff_status": "Signed Off" if r.status == PDStatus.COMPLETED_CLOSED
                                   else "Pending" if r.status == PDStatus.AWAITING_HR_SIGNOFF else "—",
                "reminder_tag": tag,
        ...
        for key, label, bucket_name in (
            ...
            ("pending_certificate", "Pending Certificate Upload", "pending_certificate"),
            ...
            if key == "pending_accountability":
                items = [r for r in all_rows if r.status in PENDING_ACCOUNTABILITY_STATUSES and r.funding_type in FUNDED_TYPES]
            elif key == "ready_signoff":
                items = [r for r in all_rows if r.status == PDStatus.AWAITING_HR_SIGNOFF]
            else:
            ...
            action_center.append({
                ...
                "items": [
                    {
                        "id": r.id, "staff_name": r.staff_name, "course_name": r.course_name,
                        "due_label": _due_label(r, key, today),
                        "action": "sign_off" if key == "ready_signoff" else "send_reminder",
                        "action_label": "Sign Off" if key == "ready_signoff" else "Send Reminder",
                    }
                    for r in sample
                ],
            })
        ...
        role_settings = HRPDDashboardService._role_allocation_settings(fy, country, scoped_ids)

        # ── Compliance donuts ────────────────────────────────────────────────
        bucket_counts = {"not_started": 0, "in_progress": 0, "pending_certificate": 0, "completed": 0, "signed_off": 0, "inactive": 0}
        for r in all_rows:
        ...
        fund_utilization = {
            "accounted": accounted, "committed_not_accounted": max(0, committed - accounted),
            "uncommitted": uncommitted, "total": total_allocation,
            "accounted_pct": round(accounted / total_allocation * 100) if total_allocation else 0,
        }

        # ── Completed / sign-off snapshot ────────────────────────────────────
        completed_this_fy = sum(1 for r in all_rows if r.status == PDStatus.COMPLETED_CLOSED)
        certs_verified = sum(
            1 for r in all_rows if r.status == PDStatus.COMPLETED_CLOSED and r.certificates.filter(status="uploaded").exists()
        )
        bamboo_confirmed = sum(1 for r in all_rows if r.status == PDStatus.COMPLETED_CLOSED and r.bamboohr_uploaded)
        eligible_for_signoff = sum(1 for r in all_rows if r.status in EMPLOYEE_DONE_STATUSES + SIGNED_OFF_STATUSES)
        signoff_rate = round(completed_this_fy / eligible_for_signoff * 100) if eligible_for_signoff else 0

        # ── Upcoming / overdue ───────────────────────────────────────────────
        upcoming_30d = [
            r for r in all_rows
            if r.status in NOT_STARTED_STATUSES + IN_PROGRESS_STATUSES and r.start_date
            and 0 <= (r.start_date - today).days <= 30
        ]
        overdue_no_cert = [r for r in all_rows if r.status in PENDING_CERT_STATUSES and (today - r.end_date).days > 0]
        ...
        return {
            "fy": fy, "fy_options": fy_options(), "country": country, "locked_country": locked_country,
            "country_options": country_options,
            "role_filter": role_filter, "status_filter": status_filter, "reminder_filter": reminder_filter,
            "search": search, "eligible_roles": PD_ELIGIBLE_ROLES,
            "status_options": STATUS_BUCKET_OPTIONS, "reminder_options": REMINDER_OPTIONS,
            "kpis": kpis, "currency": currency,
            "tracker_rows": page_rows, "tracker_total": total_entries, "tracker_page": page, "tracker_pages": pages,
            "action_center": action_center,
            "role_settings": role_settings,
            "status_distribution": status_distribution, "status_distribution_total": len(all_rows),
            "fund_utilization": fund_utilization,
            "completed_this_fy": completed_this_fy, "certs_verified": certs_verified,
            "bamboo_confirmed": bamboo_confirmed, "signoff_rate": signoff_rate,
            "upcoming_30d": upcoming_30d[:8], "upcoming_30d_count": len(upcoming_30d),
            "overdue_no_cert": overdue_no_cert[:8], "overdue_no_cert_count": len(overdue_no_cert),
            "overdue_accountability": overdue_accountability[:8], "overdue_accountability_count": len(overdue_accountability),
    ...
    def _role_allocation_settings(fy: str, country: str, scoped_ids) -> dict:
        ...
        for role, label in PD_ELIGIBLE_ROLES:
            staff_qs = StaffProfile.objects.filter(user__active_role=role, country=country)
            if scoped_ids is not None:
            ...
            rows.append({
                "role": role, "label": label, "per_staff": f"{currency} {per_staff/100:,.0f}",
                "staff_count": staff_count, "total_allocated": f"{currency} {row_total/100:,.0f}",
                "currency": currency,
            })
            total_staff += staff_count
            total_allocated += row_total
        return {
            "rows": rows, "total_staff": total_staff,
            "total_allocated": f"{total_allocated/100:,.0f}",
        }

    @staticmethod
    def adjust_role_allocation(principal, *, role: str, fy: str, country: str, amount_major: float,
                               currency: str = "UGX", apply_to_existing: bool = False) -> PDRoleAllocation:
        ...
        if getattr(principal, "active_role", "") not in ("HumanResources", "Admin"):
            raise Forbidden("Only HR may adjust Professional Development role allocations.")
        ...
        pra, _ = PDRoleAllocation.objects.update_or_create(
            ...
            defaults={"annual_allocation_cents": amount_cents, "currency": currency, "set_by": principal.user_id},
        )
        if apply_to_existing:
            from apps.accounts.models import StaffProfile

            for sp in StaffProfile.objects.filter(user__active_role=role, country=country):
                ProfessionalDevelopmentAllocation.objects.update_or_create(
                    staff_id=sp.id, fy=fy,
                    defaults={"country": country, "currency": currency, "annual_allocation": amount_cents},
                )
        return pra
...
def _due_label(r: ProfessionalDevelopmentRequest, key: str, today: date) -> str:
    if key == "not_started" and r.start_date:
        ...
        return f"Start by {r.start_date:%d %b %Y}" if days >= 0 else f"Was due {r.start_date:%d %b %Y}"
