...
def compute_next_action(a, today) -> dict:
    ...
    if a.status == "completed" and a.evidence_status == "uploaded" and not a.salesforce_activity_id:
        return {
    ...
    if a.status == "completed" and a.evidence_status == "uploaded" and a.salesforce_activity_id:
        ssa_done = True
        if a.activity_type in ["baseline_ssa_visit", "school_visit_ssa_collection", "partner_ssa_collection", "cluster_training_ssa_collection"]:
        ...
        if a.activity_type in ["training", "cluster_training", "cluster_meeting", "core_training"]:
            participants_done = (a.teachers_attended or 0) + (a.leaders_attended or 0) > 0
    ...
    if a.status == "submitted" and a.ia_verification_status == "pending":
        return {
            "text": "View Status",
            "action": "view_status",
            "url": f"/my-plan/{a.id}",
    ...
    if a.status == "submitted" and a.ia_verification_status == "verified" and a.payment_status == "pending":
        return {
    ...
    # 9. Disbursed but no accountability -> Enter NetSuite Expense ID
    wfr_line = a.schedule_cost_lines.first() if hasattr(a, 'schedule_cost_lines') else None
    if wfr_line:
        adv = wfr_line.advance_requests.first()
        if adv and adv.status == "disbursed" and not adv.accountability_netsuite_id:
            return {
                "text": "Enter NetSuite ID",
                "action": "netsuite_id",
                "url": f"/activities/{a.id}/complete",
                "description": "Accountability pending"
            }
...
def get_frontend_context(principal, query: dict) -> dict:
    ...
    districts = [{"id": d.id, "name": d.name} for d in District.objects.all().order_by("name")]
    staff_users = [{"id": u.id, "name": u.name} for u in User.objects.filter(status="active", deleted_at__isnull=True).order_by("name")]
    ...
    if district_id and district_id != "All" and district_id != "all":
        qs = qs.filter(Q(school__district_id=district_id) | Q(cluster__district_id=district_id))
    ...
    planned_this_week = qs.filter(planned_month=today.month, planned_week=min(5, (today.day - 1) // 7 + 1)).count()
    ...
    visits_scheduled = qs_period.filter(activity_type__in=[
    ...
    trainings_scheduled = qs_period.filter(activity_type__in=[
        "cluster_training", "core_training", "training", "school_improvement_training",
        "cluster_training_ssa_collection"
    ]).count()
    meetings_scheduled = qs_period.filter(activity_type__in=[
        "cluster_meeting", "cluster_meeting_ssa_review"
    ]).count()
    
    total_period_count = qs_period.count()
    completed_period_count = qs_period.filter(status="completed").count()
    completion_readiness = int(completed_period_count / total_period_count * 100) if total_period_count > 0 else 0
    ...
    for a in activities:
        ...
        badges = []
        first_line = a.schedule_cost_lines.first() if hasattr(a, 'schedule_cost_lines') else None
        ...
        if a.delivery_type == "partner":
            ...
            partner_schedule_status = "Scheduled" if a.planned_date else "Pending Partner Scheduling"
        ...
        activity_data = {
            ...
            "school_sub_county": a.school.sub_county.name if a.school and a.school.sub_county else "",
            "school_cluster_name": a.cluster.name if a.cluster else (a.school.cluster_id or ""),
            "school_ssa_status": a.school.get_current_fy_ssa_status_display() if a.school else "No SSA",
            ...
            "cluster_school_count": School.objects.filter(cluster_id=a.cluster.id).count() if a.cluster else 0,
            "expected_participants": (a.teachers_attended or 0) + (a.leaders_attended or 0) + (a.other_participants or 0) or 20,
            ...
            "focus_intervention": a.get_focus_intervention_display() if a.focus_intervention else "General",
        ...
        if a.activity_type in ["school_visit", "follow_up_visit", "coaching_visit", "in_school_support", "core_visit", "baseline_ssa_visit", "school_visit_ssa_collection", "partner_ssa_collection", "core_assessment_visit"]:
            school_visits_list.append(activity_data)
        elif a.activity_type in ["cluster_training", "core_training", "training", "school_improvement_training", "cluster_training_ssa_collection"]:
        ...
        elif a.planned_date and today < a.planned_date <= today + timedelta(days=7) and a.status in ["scheduled", "in_progress"]:
    ...
    today_activities = qs.filter(planned_date=today).select_related("school", "cluster").order_by("created_at")
    upcoming_today = []
    for a in today_activities:
        ...
        upcoming_today.append({
            "id": a.id,
            "time": "08:30 AM",
            "title": a.school.name if a.school else (a.cluster.name if a.cluster else "Activity"),
            "purpose": a.activity_purpose_text or a.get_activity_type_display(),
            "district": a.school.district.name if a.school else (a.cluster.district.name if a.cluster else "Unknown"),
    ...
    rescheduled_acts = qs.filter(reschedule_count__gt=0).select_related("school", "cluster")[:3]
    for r in rescheduled_acts:
        attention_needed.append({
            "id": r.id,
            "type": "Rescheduled",
            "badge_class": "bg-orange-50 text-orange-700 border-orange-200",
            "title": r.school.name if r.school else (r.cluster.name if r.cluster else "Activity"),
            "issue": f"Rescheduled {r.reschedule_count} times.",
            "detail": f"New date: {r.planned_date.strftime('%b %d, %Y') if r.planned_date else 'N/A'}",
        })
        
    # 2. Awaiting Evidence
    awaiting_evidence_acts = qs.filter(status="completed", evidence_status="none").select_related("school", "cluster")[:3]
    for ae in awaiting_evidence_acts:
        attention_needed.append({
            "id": ae.id,
            "type": "Awaiting Evidence",
            "badge_class": "bg-amber-50 text-amber-700 border-amber-200",
            "title": ae.school.name if ae.school else (ae.cluster.name if ae.cluster else "Activity"),
    ...
    # 3. SF ID Missing
    sf_missing_acts = qs.filter(status="completed", salesforce_activity_id__isnull=True).select_related("school", "cluster")[:3]
    for sf in sf_missing_acts:
        attention_needed.append({
            "id": sf.id,
            "type": "Activity SF ID Missing",
            "badge_class": "bg-red-50 text-red-700 border-red-200",
            "title": sf.school.name if sf.school else (sf.cluster.name if sf.cluster else "Activity"),
