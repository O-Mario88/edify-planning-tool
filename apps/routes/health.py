...
def route_intelligence_checks() -> dict:
    ...
    planned_days = set(
        Activity.objects.filter(
            deleted_at__isnull=True, activity_type="school_visit",
            delivery_type="staff", scheduled_date__date__gte=today,
        )
        .exclude(status__in=["cancelled", "not_planned", "rejected"])
        .exclude(daily_visit_batch__isnull=True)
        .values_list("daily_visit_batch__responsible_user", "daily_visit_batch__visit_date")
    )
    routed_days = set(
        DailyVisitRouteBatch.objects.filter(visit_date__gte=today)
        .values_list("responsible_user", "visit_date")
    )
    visits_without_route_batch = len(planned_days - routed_days)

    # 2. School missing coordinates AND weak/absent shipping address.
    schools_weak_location = School.objects.filter(
        Q(latitude__isnull=True) | Q(longitude__isnull=True),
        sub_county__isnull=True, parish__isnull=True,
    ).filter(Q(shipping_address__isnull=True) | Q(shipping_address="")).count()
    ...
    mixed_district_days = batches.filter(issues__code="mixed_district_types").distinct().count()
    # 4. Secondary districts grouped without an approved route group.
    ungrouped_secondary_days = batches.filter(issues__code="secondary_group_unapproved").distinct().count()
    ...
    for rb in batches.filter(target_snapshot__isnull=False).select_related("cost_batch"):
    ...
    count_mismatch = 0
    for rb in DailyVisitRouteBatch.objects.exclude(cost_batch__isnull=True).select_related("cost_batch"):
        if rb.cost_batch and rb.school_count != rb.cost_batch.school_count:
            count_mismatch += 1
    # 8. Low location confidence on a scheduled visit day.
    low_confidence_days = batches.filter(confidence__in=["low", "needs_cleanup"]).count()
