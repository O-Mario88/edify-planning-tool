...
class PlanningReadinessService:
    @staticmethod
    def get_school_readiness(school, has_catalogue, has_scheduled, partner_assignment, weakest_area):
        # 1. Cluster check
...
class ClusterRecommendationService:
    @staticmethod
    def get_cluster_recommendation(cluster, avg_ssa, weakest_interventions, school_count, assessed_count):
        ...
        if coverage_pct < 50:
            return {
                ...
                "availableActions": ["Schedule Cluster Meeting + SSA Review", "Schedule Cluster Training + SSA Collection", "Assign Partner SSA Collection"],
            }
        
        weakest = weakest_interventions[0] if weakest_interventions else "Teaching & Learning"
        return {
            ...
            "availableActions": ["Schedule Cluster Training", "Schedule Cluster Meeting", "Assign Partner", "View Cluster SSA Profile"],
        }
...
class PlanningDashboardService:
    ...
    def get_dashboard_data(principal, filters: dict):
        ...
        active_tab = filters.get("tab", "client") # client, core, clusters, partner, scheduled
        ...
        active_activities_school_ids = Activity.objects.filter(
            deleted_at__isnull=True,
            fy=fy
        ).exclude(
            status__in=["cancelled", "deferred", "not_planned", "planned"]
        ...
        if active_tab == "client":
            table_schools_qs = schools_qs.filter(school_type="client").exclude(id__in=exclude_school_ids)
        ...
        if active_tab == "clusters":
            ...
            scheduled_cluster_ids = Activity.objects.filter(
                deleted_at__isnull=True,
                fy=fy,
                activity_type__in=["cluster_meeting", "cluster_training"],
                cluster_id__isnull=False
            ).exclude(status="cancelled").values_list("cluster_id", flat=True).distinct()
            
            ready_clusters_qs = Cluster.objects.filter(
                id__in=scoped_cluster_ids,
                deleted_at__isnull=True
            ...
            clusters_slice = list(ready_clusters_qs.select_related("district").order_by("name")[start_idx:end_idx])
            ...
            for c in clusters_slice:
                ...
                paginated_clusters.append({
                    ...
                    "weakest_interventions": weakest_intervs,
                    "school_count": member_schools.count(),
                    "readiness": "Ready" if member_schools.count() > 0 else "No Schools",
            ...
            c_ids = list(set(s.cluster_id for s in paginated_schools if s.cluster_id))
            cluster_staff_map = {c.id: c.responsible_staff_id for c in Cluster.objects.filter(id__in=c_ids)}
            ...
            for s in paginated_schools:
                ...
                schools_data.append({
                    ...
                    "my_plan_status": scheduled_map[s.id][0].status if has_scheduled and len(scheduled_map[s.id]) > 0 else None,
                    "isScheduled": has_scheduled,
                    "scheduledActivityId": scheduled_map[s.id][0].id if has_scheduled and len(scheduled_map[s.id]) > 0 else None,
                    "blockedReason": readiness_details["reason"] if readiness_details["blockedActions"] else None,
                    "ownerId": s.account_owner_id,
                    "ownerName": s.account_owner_name_raw or s.account_owner_id or "—",
                    "currentPartnerType": partner_assignment.partner.name if partner_assignment and partner_assignment.partner else "None",
                })
        ...
        cost_blocked_count = 0 if has_catalogue else base_schools_qs.exclude(Q(cluster_id__isnull=True) | Q(cluster_id="")).count()
        
        cleanup_qs = base_schools_qs.exclude(Q(cluster_id__isnull=True) | Q(cluster_id="")).filter(
            Q(district_id__isnull=True) | Q(sub_county_id__isnull=True) | Q(account_owner_id__isnull=True) | Q(school_id__isnull=True) | Q(school_id="")
        )
        data_cleanup_count = cleanup_qs.count()
        clean_schools_qs = base_schools_qs.exclude(Q(cluster_id__isnull=True) | Q(cluster_id="")).exclude(id__in=cleanup_qs.values_list("id", flat=True))
        
        scheduled_schools_ids_fy = set(Activity.objects.filter(
            deleted_at__isnull=True,
            fy=fy
        ...
        schools_with_ssa_ids = set(SsaRecord.objects.filter(
        ...
        core_package_gaps_count = base_schools_qs.filter(
        ...
        scoped_cluster_ids = list(base_schools_qs.exclude(Q(cluster_id__isnull=True) | Q(cluster_id="")).values_list("cluster_id", flat=True).distinct())
        planned_cluster_ids = Activity.objects.filter(
            cluster_id__in=scoped_cluster_ids,
            deleted_at__isnull=True,
            fy=fy
        ).values_list("cluster_id", flat=True)
        clusters_needing_action_count = len(set(scoped_cluster_ids) - set(planned_cluster_ids))
        ...
        kpis = {
            ...
            "without_ssa_pct": 100,
            "unclustered": base_schools_qs.filter(Q(cluster_id__isnull=True) | Q(cluster_id="")).count(),
            "unclustered_pct": 100,
