    ...
    def get_school_readiness(
        ...
        if partner_assignment:
            ...
            if status == "partner_scheduled":
                ...
                status == "assigned_to_partner_pending_scheduling"
                or status == "assigned"
            ):
                return {
        ...
        return {
            ...
            "reason": f"{weak_str} is the weakest verified intervention score for this school."
            if weak_str != "general"
            else "School is ready for support interventions.",
            "availableActions": [
                "Schedule visit",
                "Schedule training",
                "Assign to Partner",
...
class PlanningDashboardService:
    ...
    def get_dashboard_data(principal, filters: dict):
        ...
        if active_tab == "clusters":
            ...
            {c.id: c.responsible_staff_id for c in Cluster.objects.filter(id__in=c_ids)}
