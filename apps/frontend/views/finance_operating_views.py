...
def ready_for_advance_view(request):
    ...
    advances = Activity.objects.filter(
...
def batch_payments_view(request):
    ...
    partners = Activity.objects.filter(
        deleted_at__isnull=True,
        delivery_type="partner",
        status="ia_verified",
        payment_status__in=["pending", "ia_confirmed"],
...
def monthly_request_view(request):
    ...
    budget_lines = Activity.objects.filter(
        deleted_at__isnull=True, fy="FY25"
    ).aggregate(total=Sum("schedule_cost_lines__amount"))
