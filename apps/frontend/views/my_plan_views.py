...
def activity_detail_view(request, activity_id):
    ...
    if not RolePermissionService.can_view_record(request.user, a):
        ...
        return HttpResponseForbidden("Access Denied: You do not have permission to view this activity.")
...
def complete_drawer_view(request, activity_id):
    ...
    if not RolePermissionService.can_view_record(request.user, a):
        ...
        return HttpResponseForbidden("Access Denied: You do not have permission to access this activity drawer.")
    ...
    if act.get("status") in ("scheduled", "in_progress", "assigned_to_partner", "partner_scheduled"):
        try:
            ...
            return HttpResponse(f'<div class="p-4 bg-rose-50 text-rose-700 rounded-xl text-[12px] font-bold">Error starting completion: {str(e)}</div>', status=400)
    ...
    if a.cluster:
        ...
        cluster_schools = School.objects.filter(cluster_assignments__cluster=a.cluster, deleted_at__isnull=True).order_by("name")
...
def reschedule_drawer_view(request, activity_id):
    ...
    if a.delivery_type == "partner":
        ...
        if not staff_id:
            ...
            pa = None
            if a.cluster:
                pa = PartnerAssignment.objects.filter(cluster=a.cluster, partner_id=a.assigned_partner_id).first()
            if not pa and a.school:
                pa = PartnerAssignment.objects.filter(school=a.school, partner_id=a.assigned_partner_id).first()
            if pa:
...
def reschedule_activity_action(request, activity_id):
    ...
    if not RolePermissionService.can_view_record(request.user, a):
        ...
        return HttpResponseForbidden("Access Denied: You do not have permission to reschedule this activity.")
...
def complete_activity_action(request, activity_id):
    a = get_object_or_404(Activity, id=activity_id, deleted_at__isnull=True)
    if not RolePermissionService.can_view_record(request.user, a) or not RolePermissionService.can_upload_evidence(request.user, a):
        audit_log(
            action="unauthorized_mutation_attempt",
            subject_kind="Activity",
            subject_id=str(a.id),
        ...
        return HttpResponseForbidden("Access Denied: You are not authorized to complete this activity.")
    ...
    if request.method == "POST":
        ...
        if act.get("status") in ("scheduled", "in_progress", "assigned_to_partner", "partner_scheduled"):
            try:
                ...
                if request.headers.get("HX-Request") == "true":
                    return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error starting completion: {str(e)}</div>', status=400)
        ...
        if evidence_file:
            try:
                ...
                if request.headers.get("HX-Request") == "true":
                    return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Upload Error: {str(e)}</div>', status=400)
        ...
        if netsuite_id:
            ...
            if wfr_line:
                ...
                if adv:
                    ...
                    adv.status = "accounted"
                    adv.save(update_fields=["accountability_netsuite_id", "status"])
        ...
        if a.ssa_collection_expected:
            ...
            if ssa_collected == "yes":
                ...
                for enum_val, field_name in interventions_map.items():
                    ...
                    if not val:
                        return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: Missing score for {enum_val.replace("_", " ").title()}</div>', status=400)
                    try:
                        f_val = float(val)
                    except ValueError:
                        return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: Score for {enum_val.replace("_", " ").title()} must be numeric</div>', status=400)
                    if f_val < 0 or f_val > 10:
                        return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: Score for {enum_val.replace("_", " ").title()} must be between 0 and 10</div>', status=400)
                ...
                SsaScore.objects.bulk_create([
                    SsaScore(ssa_record=rec, intervention=s["intervention"], score=s["score"])
                ...
                if not reason:
                    return HttpResponse('<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: Please select a reason why SSA was not collected.</div>', status=400)
        ...
        try:
            ...
            if request.headers.get("HX-Request") == "true":
                return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Submission Error: {str(e)}</div>', status=400)
...
def pl_confirm_action(request, activity_id):
    ...
    if not RolePermissionService.can_review_activity(request.user, a):
        ...
        return HttpResponseForbidden("Access Denied: You do not supervise the owner of this activity.")
...
def ia_confirm_action(request, activity_id):
    ...
    if not RolePermissionService.can_verify_ia(request.user, a):
        ...
        return HttpResponseForbidden("Access Denied: You are not authorized to verify activities.")
...
def ia_return_action(request, activity_id):
    a = get_object_or_454(Activity, id=activity_id, deleted_at__isnull=True)
    if not RolePermissionService.can_verify_ia(request.user, a):
        audit_log(
            action="unauthorized_mutation_attempt",
            subject_kind="Activity",
        ...
        return HttpResponseForbidden("Access Denied: You are not authorized to return activities.")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(request, "Activity returned to CCEO/partner for correction.")
        except Exception as e:
...
def salesforce_id_action(request, activity_id):
    ...
    if request.method == "POST":
        ...
        if not salesforce_id:
            return HttpResponse('<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: SF ID cannot be blank.</div>', status=400)
...
def submit_for_review_drawer_view(request, activity_id):
    ...
    ssa_required = a.activity_type in ["baseline_ssa_visit", "school_visit_ssa_collection", "partner_ssa_collection", "cluster_training_ssa_collection"]
    has_ssa = False
    if ssa_required:
    ...
    if a.activity_type in ["training", "cluster_training", "cluster_meeting", "core_training"]:
        has_participants = (a.teachers_attended or 0) + (a.leaders_attended or 0) > 0

    all_ready = has_evidence and has_sf_id and (not ssa_required or has_ssa) and has_participants
...
def activity_timeline_drawer_view(request, activity_id):
    ...
    logs = AuditLog.objects.filter(subject_kind="Activity", subject_id=str(a.id)).order_by("-created_at")
    context = {
        "act": a,
        "logs": logs,
        "drawer_size": "md",
...
def attendance_upload_action(request, activity_id):
    ...
    if request.method == "POST":
        ...
        a.status = "completed"
        a.save(update_fields=["teachers_attended", "leaders_attended", "attended_school_ids", "status", "updated_at"])
        
        if attendance_file:
            record_upload(principal=request.user, activity_id=activity_id, kind="attendance_sheet", file_obj=attendance_file)
            a.evidence_status = "uploaded"
            a.save(update_fields=["evidence_status", "updated_at"])
    ...
    if request.method == "POST":
        ...
        for sf in score_fields:
            ...
            if val:
                has_scores = True
                try:
                    scores_list.append({"intervention": sf.replace("score_", ""), "score": float(val)})
                except ValueError:
                    return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: Score must be numeric</div>', status=400)
                    
        if has_scores:
            if len(scores_list) < 8:
                return HttpResponse(f'<div class="p-3 bg-rose-50 text-rose-700 rounded-lg text-[12px] font-bold">Error: All 8 scores must be provided if manual entry is selected.</div>', status=400)
        ...
        if ssa_file:
            record_upload(principal=request.user, activity_id=activity_id, kind="ssa_form", file_obj=ssa_file)
            a.evidence_status = "uploaded"
            a.save(update_fields=["evidence_status", "updated_at"])
...
def evidence_center_view(request):
    ...
    activities = Activity.objects.filter(deleted_at__isnull=True).select_related("school").order_by("-updated_at")
