...
def cluster_list_view(request):
    ...
    cost_preview = None
    if selected_cluster:
        cost_preview = ClusterCostPreviewService.preview_cost("training", 50, selected_cluster["id"])
    ...
    if selected_cluster:
        ...
        schools = School.objects.filter(cluster_id=selected_cluster["id"], deleted_at__isnull=True)
        cluster_progress = get_ssa_progress_by_fy(schools)

    # Impact data
    impact_data = None
    if selected_cluster:
        impact_data = ClusterImpactService.get_impact_data(selected_cluster["id"], "leadership", user)
...
def cluster_schedule_activity_view(request):
    if not RolePermissionService.can_schedule_activity(request.user):
        return HttpResponseForbidden("Access Denied: You do not have permission to schedule cluster activities.")
    if request.method == "POST":
        ...
        try:
            ...
            if request.headers.get("HX-Request") == "true":
                ...
                rec = None
                if selected_cluster:
                    rec = ClusterRecommendationService.get_recommendation(selected_cluster.id, request.user)
                    
                staff_profiles = StaffProfile.objects.filter(deleted_at__isnull=True).select_related("user").order_by("user__name")
                from apps.partners.models import Partner
                partners = Partner.objects.filter(deleted_at__isnull=True)
                from apps.core.enums import SsaIntervention
                interventions = [{"value": key.value, "label": key.label} for key in SsaIntervention]
                
                cost_preview = None
                if selected_cluster:
                    try:
                        cost_preview = ClusterCostPreviewService.preview_cost(activity_type, participants, selected_cluster.id)
                    except Exception:
                        pass
                ...
                return render(request, "partials/clusters/cluster_action_planner_drawer.html", context)
...
def create_cluster_view(request):
    if not RolePermissionService.can_schedule_activity(request.user):
        return HttpResponseForbidden("Access Denied: You do not have permission to create clusters.")
    if request.method == "POST":
        ...
        if name and district_id:
            ...
            try:
                ...
                if assign_school_id and cluster_id:
                    ...
                    school = get_scoped_object_or_404(School, request.user, id=assign_school_id, deleted_at__isnull=True)
                    cluster = get_scoped_object_or_404(Cluster, request.user, id=cluster_id, deleted_at__isnull=True)
                    ...
                    messages.success(request, f"School '{school.name}' has been assigned to the new cluster '{cluster.name}'.")
...
def planner_drawer_view(request):
    ...
    if selected_cluster:
        ...
        try:
            weakest_interventions = cluster_weakest_interventions(selected_cluster.id, request.user)
        except Exception:
            pass
...
def intervention_impact_drawer_view(request, cluster_id):
    focus_intervention = request.GET.get("focus_intervention", "leadership").strip()
    impact_data = ClusterImpactService.get_impact_data(cluster_id, focus_intervention, request.user)
    context = {
        "cluster_id": cluster_id,
        "focus_intervention": focus_intervention,
        "impact_data": impact_data,
...
def cluster_bulk_assign_drawer_view(request, cluster_id):
    ...
    cluster = get_scoped_object_or_404(Cluster, request.user, id=cluster_id, deleted_at__isnull=True)
    covered_sub_counties = ClusterSubCounty.objects.filter(cluster=cluster).values_list("sub_county_id", flat=True)
        ...
        for sid in school_ids:
            ...
            if school:
                ...
                audit_log(
                    ...
                    success=True,
                    payload={"cluster_id": cluster.id, "cluster_name": cluster.name, "bulk": True}
                )
        
        msg = f"Successfully assigned {len(assigned_schools)} schools to {cluster.name}."
        response = render(request, "partials/schools/toast_success.html", {"message": msg})
