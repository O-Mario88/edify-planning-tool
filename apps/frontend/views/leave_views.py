...
from apps.accounts.models import Leave, LeaveTypePolicy, LeaveBalance, TemporaryCoverageAssignment, StaffProfile, PublicHoliday, User, CalendarBlock
...
from apps.hr.leave_services import (
    ...
    CalendarBlockService, PublicHolidayService, BlackoutDateService,
    LeaveConflictDetectionService, PlanningAvailabilityService, LeaveImpactPreviewService,
    TeamAvailabilityService, LeaveNotificationService, LeaveBudgetImpactService
)
...
def personal_time_off_view(request):
    ...
    balances = LeaveBalance.objects.filter(staff=sp, year=year).select_related("staff__user")
    ...
    for b in balances:
        balances_data.append({
            "type": b.leave_type,
            "label": policy_map.get(b.leave_type, b.leave_type.replace("_", " ").title()),
            "entitlement": b.entitlement,
            "used": b.used,
            "pending": b.pending,
            "remaining": b.remaining,
            "pct_used": round((b.used / max(b.entitlement, 1)) * 100) if b.entitlement else 0
        })
        if b.leave_type == "personal_time_off":
            pto_remaining = b.remaining

    # Query user leaves history
    leaves = Leave.objects.filter(staff=sp).order_by("-created_at").select_related("covering_staff__user")
    ...
    for l in leaves:
        try:
            s_dt = datetime.strptime(l.start_date, "%Y-%m-%d")
            e_dt = datetime.strptime(l.end_date, "%Y-%m-%d")
        ...
        leaves_data.append({
            "id": l.id,
            "type_label": policy_map.get(l.type, l.type.replace("_", " ").title()),
            "start_date": l.start_date,
            "end_date": l.end_date,
            "day_str": day_str,
            "days": l.days,
            "days_charged": l.days_charged if l.days_charged is not None else l.days,
            "hours_covered": l.hours_covered if l.hours_covered is not None else (l.days * 8),
            "status": l.status,
            "reason": l.reason,
            "cover_name": l.covering_staff.user.name if l.covering_staff else "None",
    ...
    supervisee_ids = StaffSupervisorAssignment.objects.filter(supervisor=sp).values_list("supervisee_id", flat=True)
    supervisee_profiles = StaffProfile.objects.filter(id__in=supervisee_ids).select_related("user")
    ...
    coverage_active_count = TemporaryCoverageAssignment.objects.filter(
        status="active"
    ).filter(Q(original_staff=sp) | Q(covering_staff=sp)).count()

    # 1. Holiday & Blackout Dates
    upcoming_blocks = CalendarBlock.objects.filter(is_active=True, end_date__gte=date.today()).order_by("start_date")[:5]

    # 2. Auto-Blocked Conflicts (for the user's activities)
    user_activities = Activity.objects.filter(responsible_staff_id=user.id, scheduled_date__isnull=False).exclude(status__in=["cancelled", "completed"])
    my_conflicts = []
    for act in user_activities:
        ...
        if avail["status"] == "blocked":
            my_conflicts.append({
                ...
                "school": act.school.name if act.school else (act.cluster.name if act.cluster else "General"),
    ...
    avg_team_availability = round((team_available_count / max(total_team_count, 1)) * 100)

    # 3. Team Availability Heatmap Preview (Next 4 weeks)
    availability_preview = TeamAvailabilityService.get_4week_heatmap(sp, country_scope=False)[:10]

    # Leave Impact Preview for latest request
    latest_leave = Leave.objects.filter(staff=sp).order_by("-created_at").first()
    impact_preview = None
    if latest_leave:
        impact_preview = LeaveImpactPreviewService.preview_impact(sp, latest_leave.start_date, latest_leave.end_date)
    ...
    if supervisee_profiles.exists():
        ...
        supervisor_assignment = StaffSupervisorAssignment.objects.filter(supervisee=sp).first()
    ...
    tracker_leaves = tracker_leaves_qs.select_related("staff__user", "covering_staff__user").order_by("-created_at")[:15]
    
    # Enrich tracker leaves with remaining balance
    tracker_leaves_data = []
    for tl in tracker_leaves:
        bal = LeaveBalance.objects.filter(staff=tl.staff, leave_type=tl.type, year=year).first()
        tracker_leaves_data.append({
            "id": tl.id,
            "staff_name": tl.staff.user.name,
            "role": tl.staff.user.active_role,
            "supervisor_name": reviewer_map.get(tl.reviewed_by_user_id, "System") if tl.reviewed_by_user_id else "Pending Review",
            "type_label": policy_map.get(tl.type, tl.type.replace("_", " ").title()),
            "start_date": tl.start_date,
            "end_date": tl.end_date,
            "days_charged": tl.days_charged if tl.days_charged is not None else tl.days,
            "remaining_days": bal.remaining if bal else 21,
            "covering_person": tl.covering_staff.user.name if tl.covering_staff else "None",
            "status": tl.status
        })

    # Calendar highlighted events map for the widget
    calendar_events = []
    for l in Leave.objects.filter(status__in=["approved", "pending"]):
...
def request_leave_drawer_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(request, f"Leave request for {leave.days_charged} working days submitted successfully.")
            
            # Send notifications
            LeaveNotificationService.notify_leave_approved(leave)
            
            # Detect conflicts post request and alert
            conflicts = LeaveConflictDetectionService.detect(sp, leave.start_date, leave.end_date, leave.covering_staff)
            if conflicts:
                msg = f"{len(conflicts)} scheduling conflicts detected for your requested leave period. Please reschedule them."
                LeaveNotificationService.notify_conflict_detected(user.id, msg, leave.id)
    ...
    candidates = CoverageAssignmentService.get_eligible_coverage_staff(sp, tomorrow, tomorrow)
...
def eligible_cover_api(request):
    ...
    try:
        candidates = CoverageAssignmentService.get_eligible_coverage_staff(sp, start_date, end_date)
...
def leave_tracker_view(request):
    ...
    if q:
        qs = qs.filter(Q(user__name__icontains=q) | Q(user__email__icontains=q) | Q(title__icontains=q))
    ...
    for sp in qs:
        ...
        pto_bal = LeaveBalance.objects.filter(staff=sp, leave_type="personal_time_off", year=year).first()
        sick_bal = LeaveBalance.objects.filter(staff=sp, leave_type="sick_leave", year=year).first()
        ...
        next_leave = Leave.objects.filter(
            staff=sp,
            status="approved",
            start_date__gt=now_str
        ...
        team_data.append({
            ...
            "covering_person": active_leave.covering_staff.user.name if active_leave and active_leave.covering_staff else "None",
        })

    # Current coverage assignments
    coverages = TemporaryCoverageAssignment.objects.filter(
        status="active"
    ).select_related("original_staff__user", "covering_staff__user", "leave_request").order_by("start_datetime")

    if role == "PL":
        sp_id = user.staff_profile.id
        coverages = coverages.filter(Q(original_staff_id=sp_id) | Q(covering_staff_id=sp_id))

    # Next 4 weeks availability preview
    availability_preview = TeamAvailabilityService.get_4week_heatmap(getattr(user, "staff_profile", None), country_scope=(role in ["CD", "HR", "ADMIN"]))[:10]
...
def leave_approvals_view(request):
    """Queue of leave requests awaiting review."""
    user = request.user
    role = get_user_role_slug(user)
    
    qs = Leave.objects.filter(status="pending").select_related("staff__user", "covering_staff__user").order_by("-created_at")
    ...
    if role == "PL":
        ...
        qs = qs.filter(staff_id__in=supervisee_ids)
    elif role == "CD":
        qs = qs.filter(staff__user__active_role__in=["Program Lead", "ImpactAssessment", "Accountant"])
    elif role == "RVP":
        qs = qs.filter(staff__user__active_role="CountryDirector")
...
def leave_impact_partial(request, leave_id):
    ...
    impact = LeaveImpactAnalysisService.analyze_impact(leave.staff, leave.start_date, leave.end_date)
    
    # Run full conflict detection service
    conflicts = LeaveConflictDetectionService.detect(leave.staff, leave.start_date, leave.end_date, leave.covering_staff)
    
    # Extract overlaps and schedule conflicts from detected list
    overlaps = [c for c in conflicts if c["conflict_type"] in ["public_holiday", "blackout_date", "staff_conference", "cover_unavailable"]]
    act_conflicts = [c for c in conflicts if c["conflict_type"] == "activity_during_leave"]
...
def leave_approve_action(request, leave_id):
    ...
    try:
        ...
        LeaveNotificationService.notify_leave_approved(leave)
        messages.success(request, f"Leave approved. Coverage notifications sent to the team.")
    except Exception as e:
...
def leave_return_action(request, leave_id):
    ...
    try:
        ...
        messages.success(request, "Leave request returned to submitter for changes.")
    except Exception as e:
...
def leave_coverage_view(request):
    ...
    coverages = TemporaryCoverageAssignment.objects.all().select_related(
        "original_staff__user", "covering_staff__user", "leave_request"
    ).order_by("-created_at")
...
def revoke_coverage_action(request, assignment_id):
    ...
    messages.success(request, f"Delegated access for {cov.covering_staff.user.name} has been revoked.")
...
def leave_calendar_view(request):
    ...
    approved = Leave.objects.filter(status="approved").select_related("staff__user", "covering_staff__user")
    pending = Leave.objects.filter(status="pending").select_related("staff__user", "covering_staff__user")
    coverages = TemporaryCoverageAssignment.objects.filter(status="active").select_related("original_staff__user", "covering_staff__user")
    ...
    for l in approved:
        events.append({
            "title": f"🏖 {l.staff.user.name} - {l.type.replace('_', ' ').title()}",
            "start": l.start_date,
            "end": (datetime.strptime(l.end_date, "%Y-%m-%d") + timezone.timedelta(days=1)).date().isoformat(),
            "color": "#10b981", # Teal/Green
            "textColor": "#ffffff",
            "extendedProps": {
                "type": "Approved Leave",
                "staff": l.staff.user.name,
                "cover": l.covering_staff.user.name if l.covering_staff else "None",
            }
        })
    ...
    for l in pending:
        events.append({
            "title": f"⏳ Pending: {l.staff.user.name}",
            "start": l.start_date,
            "end": (datetime.strptime(l.end_date, "%Y-%m-%d") + timezone.timedelta(days=1)).date().isoformat(),
...
def leave_policies_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            policy.requires_attachment = request.POST.get("requires_attachment") == "yes"
            ...
            policy.public_holidays_count = request.POST.get("public_holidays_count") == "yes"
            policy.save()
            messages.success(request, f"Leave policy for '{policy.label}' updated successfully.")
        except Exception as e:
    ...
    if request.GET.get("action") == "add_holiday" and request.GET.get("holiday_name") and request.GET.get("holiday_date"):
        try:
    ...
    context = {
        "policies": policies,
        "holidays": holidays,
        "approver_roles": ["Program Lead", "CountryDirector", "RegionalVicePresident", "HumanResources", "Admin"]
    }
...
def public_holidays_view(request):
        ...
        if action == "create":
            try:
                ...
                data = {
                    ...
                    "applies_to_all_roles": request.POST.get("applies_to_all_roles") == "yes",
                }
    ...
    if request.GET.get("action") == "seed" and is_editor:
        ...
        messages.success(request, "Default Uganda holidays and calendar blocks seeded successfully.")
        return redirect("frontend:public_holidays")

    # Fetch and group blocks
    blocks = CalendarBlock.objects.filter(is_active=True).order_by("start_date")
    
    upcoming_holidays = blocks.filter(block_type="PUBLIC_HOLIDAY", end_date__gte=date.today())
...
def team_availability_view(request):
    ...
    weeks_headers = []
    for i in range(4):
        start = today + timezone.timedelta(weeks=i) - timezone.timedelta(days=today.weekday())
        end = start + timezone.timedelta(days=6)
        weeks_headers.append(f"Wk {i+1} ({start.strftime('%d %b')} - {end.strftime('%d %b')})")
