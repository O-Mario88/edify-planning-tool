    ...
    if request.method == "POST":
        ...
        if not user or not password_ok or not user.is_active:
            ...
            if user:
                ...
                max_failed = getattr(settings, "AUTH_MAX_FAILED_LOGINS", 10)
    ...
    try:
        ...
        for admin in admin_users:
            Notification.objects.create(
                ...
                recipient_role=EdifyRole.ADMIN.value,
                title=f"Account Locked: {locked_user.name}",
                body=f"{locked_user.email} has been locked after {getattr(settings, 'AUTH_MAX_FAILED_LOGINS', 10)} failed login attempts. Go to User Management to unlock.",
                context_type="user",
                context_id=locked_user.id,
                target_route=f"/admin-panel/users/{locked_user.id}",
                action_label="Unlock Account",
                action_required=True,
                priority="high",
                source_event_type="account_lockout",
                source_event_id=locked_user.id,
            )
    except Exception:
        import logging
        logging.getLogger("edify.auth").exception("Failed to notify admins of account lockout")
...
def force_change_password_view(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Your password has been updated successfully. Welcome!")
        return redirect("/dashboard")
