...
def closure_readiness_queue_view(request):
    ...
    activities = Activity.objects.filter(deleted_at__isnull=True).exclude(
        status__in=["not_planned", "planned", "scheduled", "assigned_to_partner", "partner_scheduled"]
    ).select_related("school", "cluster", "closure_checklist").order_by("-updated_at")
...
def close_activity_action(request, activity_id):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(request, f"Activity #{a.id[:8]} closed, locked, and moved to Completed Activities.")
...
def blocked_closure_view(request):
    ...
    blockers = ClosureBlocker.objects.all().select_related("activity", "activity__school")
...
def reopen_activity_action(request, activity_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(request, f"Activity #{a.id[:8]} reopened and returned to responsible queues.")
        except Exception as e:
...
def analytics_publishing_status_view(request):
    ...
    records = AnalyticsPublishRecord.objects.all().select_related("activity", "activity__school")
