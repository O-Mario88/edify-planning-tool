...
def detail_group(activity, summary_group):
    if summary_group == 'non_school':
        return 'non_school'
    if activity.activity_type in (*CLUSTER_MEETING_TYPES, 'cluster_training', 'cluster_training_ssa_collection'):
        return 'cluster'
...
def detail_fields(activity, summary_group, period_label):
    ...
    def participants(per_school):
        return invited * per_school if invited is not None and per_school is not None else None
    ...
    return {
        ...
        'school_name': activity.school.name if activity.school_id else 'School not recorded',
        'school_code': activity.school.school_id if activity.school_id else '',
        'cluster_name': activity.cluster.name if activity.cluster_id else 'Cluster not recorded',
...
def grouped_tables(rows):
    return [
        {'key': key, 'title': title, 'page_param': f'wp_{key}_page',
         'rows': [r for r in rows if r['detail_group'] == key]}
        for key, title in [('school', 'School visits'), ('cluster', 'Cluster activities'),
                           ('non_school', 'Non-school activities')]
    ]


def work_plan_action(activity, *, owned, enabled, recipient=None, recipient_name='responsible person'):
    """Keep closed records readable; scheduling edits the existing activity."""
    if activity.status in COMPLETED_WORK_STATUSES or activity.status in ('cancelled', 'rejected', 'deferred'):
        return {'text': 'View', 'url': f'/my-plan/{activity.id}', 'description': 'View activity record'}
    if owned and enabled and activity.delivery_type != 'partner' and activity.status in ('draft', 'planned', 'scheduled', 'rescheduled'):
        return {'text': 'Schedule', 'url': f'/my-plan/{activity.id}/reschedule-drawer',
                'drawer': True, 'description': 'Schedule or change the date of this existing activity'}
    if not owned and enabled and recipient:
        return {'text': f'Send to {recipient_name}',
                'url': '/messages/new?' + urlencode({'context_type': 'activity',
                       'context_id': activity.id, 'to': recipient}),
