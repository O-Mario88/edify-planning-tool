...
class BudgetSpecificationTest(TestCase):
    ...
    def test_minimum_preview_and_operational_funding_use_the_same_planned_quantities(
        ...
        # its materials and the staff member's day, transport and lunch; at
        # the minimum rates above that is 14,000 + 10,000 + 10 × 3,000 +
        # 5,000 + 10,000 (the TOT rate and the materials default to 0).
        #
        # The 10,000 staff lunch is in: the participants' meals are the
        # session's line and the staff member's lunch is the day's (session
        # costing spec, 2026-09-26). From 2026-09-17 until then a session
        # that fed its participants dropped the lunch (the owner's answer to
        # "Cluster training is fetching 2 costs for meals").
        ...
        self.assertEqual(
            ...
            # 276,000 at operational rates: 56,000 transport + 30,000 lunch
            # + 10 × 12,000 meals + 30,000 facilitation + 40,000 venue.
            276000,
        )
        wfr = self.request()
    ...
    def test_cd_edits_both_rates_and_preserves_previous_published_version(self):
        ...
        # 69,000 minimal recipe (participant meals, facilitation, venue, and
        # the staff member's transport and lunch, see above) plus the
