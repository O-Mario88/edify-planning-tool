...
class KpiStripRenderingTests(SimpleTestCase):
    def render(self, items, **context):
        return render_to_string(
            "components/context_metrics.html", {"items": items, **context}
        )
    ...
    def test_all_eight_metrics_and_real_helpers_are_retained(self):
        output = self.render(
            [
                {"label": f"Metric {i}", "value": i, "helper": "Within plan"}
                for i in range(8)
            ]
        )
    ...
    def test_trend_stays_visible_without_a_tone_and_keeps_helper_context(self):
        item = {
            "label": "Schools reached",
            "value": 3,
            "helper": "Verified schools in scope",
            "trend": {"direction": "up", "value": "+2 vs FY2025"},
        }
        output = self.render([item])
        self.assertIn("+2 vs FY2025</span>", output)
        self.assertIn('title="+2 vs FY2025 · Verified schools in scope"', output)
        self.assertEqual(output.count('class="context-metrics__helper"'), 1)
        self.assertNotIn("tone", item)
            ...
            [
                {
                    "label": "Coverage",
                    "value": "—",
                    "data_state": "not_collected",
                    "is_measured": False,
                    "metric_key": "coverage",
                    "period": "FY27",
                    "scope": "Uganda",
                    "accessible_description": "Coverage has not been collected.",
                }
            ]
        )
        for text in [
            "context-metrics__fact--pending",
            'data-metric-key="coverage"',
            'data-metric-period="FY27"',
            'data-metric-scope="Uganda"',
            "Coverage has not been collected.",
        ]:
            ...
            [
                {
                    "label": "Schools",
                    "value": 3,
                    "drilldown_url": "/schools/?type=core",
                },
                {"label": "Approvals", "value": 7, "hx_get": "/approvals/drawer"},
            ]
        )
    ...
    def test_labels_and_helpers_are_escaped_and_trends_are_never_invented(self):
        output = self.render(
            [{"label": "<script>bad</script>", "value": 2, "helper": "<b>test</b>"}]
        )
...
class PlatformMetricBlockTests(SimpleTestCase):
    ...
    def test_loop_conditions_values_helpers_and_links_keep_their_scope(self):
        ...
        html = self.render(
            ...
            rows=[
                ...
                dict(label="Hidden", visible=False),
            ],
        )
    ...
    def test_missing_value_remains_unknown_and_compact_density_keeps_all_metrics(self):
        html = self.render(
            "{% kpi_strip %}{% kpi_metric %}{% kpi_label %}Coverage{% endkpi_label %}{{ missing }}{% endkpi_metric %}{% endkpi_strip %}"
        )
        self.assertIn(">—</strong>", html)
        html = render_to_string(
            "components/context_metrics.html",
            {
                "items": [dict(label=str(i), value=i) for i in range(9)],
                "density": "compact",
            },
        )
    ...
    def test_all_platform_templates_compile(self):
        ...
        for path in (Path(settings.BASE_DIR) / "templates").rglob("*.html"):
            with self.subTest(template=str(path)):
                get_template(
                    str(path.relative_to(Path(settings.BASE_DIR) / "templates"))
                )
    ...
    def test_large_currency_has_compact_display_and_exact_accessible_amount(self):
        html = render_to_string(
            "components/context_metrics.html",
            {"items": [dict(label="Approved", value="UGX 1,250,000,000")]},
        )
...
class KpiInterventionAbbreviationTests(SimpleTestCase):
    """Owner, 2026-09-12: "abbreviate the intervention names inside the KPIs …
    'Exposure to the Word of God' could be 'WOG' … in all the KPIs and stats
    strips in the entire platform." The tile shows the code; its title keeps
    the whole name."""
