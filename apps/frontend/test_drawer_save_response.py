...
class PartnerRosterDrawerResponseTest(SimpleTestCase):
    def request(self):
        request = RequestFactory().post('/partners/example/members', {'name': 'Grace'}, HTTP_HX_REQUEST='true')
        request.user = SimpleNamespace(id='example-user')
        request.session = {}
        request._messages = FallbackStorage(request)
        return request

    @patch('apps.partners.services.add_member', side_effect=BadRequest('Name is required.'))
    ...
    @patch('apps.partners.services.add_member', return_value=SimpleNamespace(name='Grace'))
