...
from apps.budget.costing_service import active_catalogue, activity_cost_coverage, preview
...
class LinkedCostTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.cd = User.objects.create(
            id="cd_link", email="cd.link@edify.org", name="CD Link", roles=["CountryDirector"]
        )
        cls.cd.active_role = "CountryDirector"
        cls.visit = ActivityCatalogueItem.objects.get(stable_code="STANDARD_SCHOOL_VISIT")
        cls.other = ActivityCatalogueItem.objects.exclude(pk=cls.visit.pk).order_by("display_name").first()
    ...
    def test_the_activity_prices_the_cost_and_other_activities_do_not(self):
        self._add()
        base = {"activityType": "school_visit", "deliveryType": "staff", "districtType": "primary"}
        ...
        self.assertNotIn(
            "activity:standard_school_visit:diagnostic_kit", [line["key"] for line in without["lines"]]
        )
    ...
    def test_the_cost_shows_in_the_activity_coverage_and_the_rate_list(self):
        ...
        self.assertNotIn("Diagnostic kit", coverage[self.other.stable_code]["components"])
    ...
    def test_editing_the_cost_keeps_its_link_across_versions(self):
        ...
        budget_services.upsert_cost_setting(
            {"key": result["key"], "unitCost": 30_000, "reason": "Kits cost more."}, self.cd
        )
        row = CostSetting.objects.get(key=result["key"], catalogue=active_catalogue())
