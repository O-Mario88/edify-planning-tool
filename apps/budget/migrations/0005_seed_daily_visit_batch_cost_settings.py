...
DEFAULT_DAILY_BATCH_SETTINGS = [
    ...
    ("secondary_accommodation_per_night", "Secondary district accommodation per night", 40000),
    ("secondary_overnight_dinner_per_day", "Secondary district overnight dinner", 12000),
    ("secondary_breakfast_per_day", "Secondary district breakfast (optional)", 8000),
    ("secondary_incidentals_per_day", "Secondary district incidentals (optional)", 5000),
]
...
def _seed_daily_batch_keys(apps, schema_editor):
    ...
    for key, label, cost in DEFAULT_DAILY_BATCH_SETTINGS:
        CostSetting.objects.get_or_create(
            key=key,
            defaults={"label": label, "unit_cost": cost, "fy": fy, "catalogue": active, "version": 1},
        )
