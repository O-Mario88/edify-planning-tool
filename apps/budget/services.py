...
def from_schedule(principal, query: dict) -> dict:
    ...
    for a in activities:
        ...
        if a.planned_month:
            by_month_amount[a.planned_month] = (
                by_month_amount.get(a.planned_month, 0) + amount
            )
            by_month_count[a.planned_month] = by_month_count.get(a.planned_month, 0) + 1
            if a.activity_type in TRAINING_TYPES:
                by_month_trainings[a.planned_month] = (
                    by_month_trainings.get(a.planned_month, 0) + 1
                )
        # By delivery.
