...
class DashboardMetricsService:
    ...
    def get_dashboard_metrics(user, progress_period: str = "week"):
        ...
        # Evidence is one-to-many. Every activity count in this shared
        # aggregate must remain distinct when an activity has several uploads.
        activity_summary = activities_qs.aggregate(
            ...
            monthly_total=Count(
                "id", filter=Q(planned_month=today.month), distinct=True
            ),
            monthly_done=Count(
                "id",
                filter=Q(
            ...
            quarterly_total=Count(
                "id", filter=Q(quarter=current_quarter), distinct=True
            ),
            ...
            fy_total=Count("id", distinct=True),
            fy_done=Count(
                "id", filter=Q(status__in=COMPLETED_WORK_STATUSES), distinct=True
            ),
