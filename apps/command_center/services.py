...
def today(principal) -> dict:
    ...
    if scope.can_approve:
        ia_qs = Activity.objects.filter(
            status="awaiting_ia_verification", deleted_at__isnull=True
        )
        n = ia_qs.count()
        if n:
            action_items.append(
    ...
    if Permission.PAYMENT_ACT.value in scope.permissions:
        pay_qs = Activity.objects.filter(
            payment_status="ia_confirmed", deleted_at__isnull=True
        )
        n = pay_qs.count()
        if n:
            action_items.append(
            ...
            critical_items.append(
                {
                    ...
                    "action": {"label": "Assess", "href": "/schools"},
                    "count": n,
                }
            )
    ...
    if critical_items:
        groups.append(
            {"key": "critical", "label": "Critical Focus", "items": critical_items}
        )
    if action_items:
        groups.append({"key": "action", "label": "Next Actions", "items": action_items})
    if attention_items:
        groups.append(
            {"key": "attention", "label": "Under Review", "items": attention_items}
        )
...
def alerts(principal) -> list[dict]:
    ...
    now = timezone.now()
    dismissed_ids = set(
        CommandCenterAlertDismissal.objects.filter(
            user_id=principal.user_id, dismissed_until__gt=now
        ).values_list("alert_id", flat=True)
    )
    qs = (
        CommandCenterAlert.objects.filter(status="open")
...
def dismiss(alert_id: str, data: dict, principal) -> dict:
    ...
    CommandCenterAlertDismissal.objects.update_or_create(
        ...
        defaults={"dismissed_until": dismissed_until},
    )
    return {
        "ok": True,
        "alertId": alert_id,
        "dismissedUntil": dismissed_until.isoformat(),
    }
