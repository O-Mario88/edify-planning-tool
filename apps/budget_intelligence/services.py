...
def boards(principal, query: dict) -> dict:
    ...
    return {"fy": fy, "insights": [_serialize(i) for i in qs]}
...
def recompute(query: dict, principal) -> dict:
    ...
    ).exclude(status=DecisionStatus.NEW).exclude(reviewed_at__isnull=True).delete()
    BudgetIntelligenceInsight.objects.filter(
        fy=fy,
        scope_id__startswith="auto:",
...
def _detect_cash_variance(fy: str, now) -> list[dict]:
    ...
    for w in WeeklyFundRequest.objects.filter(
        fy=fy, accounted_amount__isnull=False, disbursed_amount__isnull=False
    ):
        disbursed = w.disbursed_amount or 0
...
def _detect_large_pending_advances(fy: str, now) -> list[dict]:
    ...
    pending = (
        WeeklyFundRequest.objects.filter(
            fy=fy, status__in=["submitted_to_cd", "submitted_to_rvp"]
        )
