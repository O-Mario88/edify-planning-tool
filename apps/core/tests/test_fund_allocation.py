...
class FundAllocationTest(TestCase):
    def setUp(self):
        ...
        self.district = District.objects.create(
            name="Allocation District", region=self.region
        )
        ...
        self.accountant = User.objects.create_user(
            email="accountant@agg.test",
            name="Moses Accountant",
    ...
    def _schedule_visit(self, month: int, user_id: str) -> Activity:
        ...
        ActivityScheduleCostLine.objects.filter(activity=a).update(
    ...
    def test_admin_budget_allocation_service(self):
        ...
        res_approved = AdminBudgetAllocationService.get_admin_budget_allocation(
            4, self.fy
        )
    ...
    def test_views_response_and_drawers(self):
        ...
        resp = self.client.get(
            reverse("frontend:fund_allocation"), {"month": "April", "fy": self.fy}
        )
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "Consolidated Fund Allocation")

        # Admin budget drilldown partial
        resp_drill = self.client.get(
            reverse("frontend:admin_budget_drilldown"),
