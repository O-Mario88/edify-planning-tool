...
def _seed_rates(**rates: int) -> None:
    ...
    for key, unit_cost in rates.items():
        CostSetting.objects.update_or_create(key=key, defaults={"label": key, "unit_cost": unit_cost})
...
class CentralizedCostingTest(APITestCase):
    def setUp(self):
        ...
        self.school = School.objects.create(
            school_id="COST-SCH", name="Cost Primary", region=self.region,
        ...
        self.cluster = Cluster.objects.create(
            name="Cost Cluster", region=self.region, district=self.district,
    ...
    def _line_sum(self, activity_id: str) -> int:
        return sum(l.amount for l in ActivityScheduleCostLine.objects.filter(activity_id=activity_id))
    ...
    def test_staff_primary_visit_transport_times_schools_plus_lunch(self):
        ...
        prev = self._preview({"activityType": "school_visit", "deliveryType": "staff", "districtType": "primary"})
        ...
        scheduled = self._post("/api/planning/schedule-school-visit", {
            "schoolId": "COST-SCH", "scheduledDate": "2026-07-10T09:00:00+03:00",
            "plannedMonth": 7, "plannedWeek": 2, "purposeIntervention": "leadership",
    ...
    def test_staff_secondary_visit_includes_meals_and_accommodation(self):
        _seed_rates(staff_visit_transport_secondary=25000, breakfast=5000, lunch=8000,
                    dinner=12000, accommodation=40000)
        prev = self._preview({
            "activityType": "school_visit", "deliveryType": "staff",
            "districtType": "secondary", "nights": 2,
        })
        self.assertTrue(prev["canSchedule"], prev)
        ...
        labels = {l["lineItemType"] for l in prev["lines"]}
        self.assertEqual(labels, {"transport", "breakfast", "lunch", "dinner", "accommodation"})
    ...
    def test_staff_primary_visit_excludes_secondary_costs(self):
        ...
        _seed_rates(staff_visit_transport_primary=15000, lunch=8000, breakfast=5000, accommodation=40000)
        prev = self._preview({"activityType": "school_visit", "deliveryType": "staff", "districtType": "primary", "nights": 3})
        labels = {l["lineItemType"] for l in prev["lines"]}
        self.assertEqual(labels, {"transport", "lunch"})
        self.assertEqual(prev["amount"], 15000 + 8000)
    ...
    def test_group_training_includes_venue_facilitation_and_group_meals(self):
        _seed_rates(training_session_fee=60000, venue=30000, meals_per_participant=6000,
                    mobilisation_per_participant=2000)
        prev = self._preview({
            "activityType": "cluster_training", "deliveryType": "staff", "expectedParticipants": 20,
        })
        self.assertTrue(prev["canSchedule"], prev)
    ...
    # ── D. Cluster meeting ───────────────────────────────────────────────────
    def test_cluster_meeting_excludes_venue_facilitation_and_uses_cluster_meal_rate(self):
        ...
        _seed_rates(cluster_meeting_cost=5000, meals_per_participant=6000,
                    venue=30000, training_session_fee=60000)
        prev = self._preview({
            "activityType": "cluster_meeting", "deliveryType": "staff", "expectedParticipants": 12,
        })
        self.assertTrue(prev["canSchedule"], prev)
        ...
        self.assertNotIn("participant_meals", labels)  # group-training rate must NOT appear
    ...
    def test_cluster_meeting_requires_participants(self):
        ...
        r = self.client.post("/api/planning/schedule-cluster-activity", {
            "activityType": "cluster_meeting", "clusterId": self.cluster.id,
            "scheduledDate": "2026-07-10T09:00:00+03:00", "plannedMonth": 7,
        }, format="json")
    ...
    def test_partner_visit_uses_partner_lump_sum(self):
        _seed_rates(partner_visit_lump_sum=35000, staff_visit_transport_primary=15000, lunch=8000)
        prev = self._preview({"activityType": "school_visit", "deliveryType": "partner"})
        self.assertTrue(prev["canSchedule"], prev)
    ...
    def test_missing_rate_blocks_scheduling(self):
        ...
        r = self.client.post("/api/budget/costing/preview", {
        ...
        sched = self.client.post("/api/planning/schedule-school-visit", {
            "schoolId": "COST-SCH", "scheduledDate": "2026-07-10T09:00:00+03:00",
            "plannedMonth": 7, "purposeIntervention": "leadership", "districtType": "primary",
        }, format="json")
    ...
    def test_advance_auto_created_and_accountant_gated_on_confirmation(self):
        ...
        self.assertTrue(all(a.status == AdvanceRequestStatus.PENDING_RESPONSIBLE_CONFIRMATION for a in advances))
        self.assertTrue(all(a.responsible_user_id == self.cceo.id for a in advances))

        accountant = User.objects.create_user(
            email="ac@cost.test", name="Ac", roles=[EdifyRole.PROGRAM_ACCOUNTANT.value],
        ...
        adv = advances[0]
        blocked = self.client.post(f"/api/fund-requests/advances/{adv.id}/disburse",
                                   {"amount": adv.amount, "method": "mobile_money"}, format="json")
        ...
        self._as(accountant)
        disbursed = self._post(f"/api/fund-requests/advances/{adv.id}/disburse",
                               {"amount": adv.amount, "method": "mobile_money", "reference": "MM-1"}, 200)
    ...
    def test_self_funded_skips_advance_and_opens_reimbursement(self):
        ...
        scheduled = self._post("/api/planning/schedule-school-visit", {
            "schoolId": "COST-SCH", "scheduledDate": "2026-07-10T09:00:00+03:00",
            "plannedMonth": 7, "purposeIntervention": "leadership",
        }, 201)
        ...
        accountant = User.objects.create_user(
            email="ac2@cost.test", name="Ac2", roles=[EdifyRole.PROGRAM_ACCOUNTANT.value],
            active_role=EdifyRole.PROGRAM_ACCOUNTANT.value, password="x", is_active=True,
        )
        self._as(accountant)
        blocked = self.client.post(f"/api/fund-requests/advances/{adv.id}/disburse",
                                   {"amount": adv.amount}, format="json")
        self.assertEqual(blocked.status_code, 400, blocked.content)

        # After completion, responsible user submits a reimbursement claim.
        self._as(self.cceo)
        claim = self._post(f"/api/fund-requests/advances/{adv.id}/submit-reimbursement",
                           {"amountSpent": adv.amount, "netsuiteId": "EXP-9"}, 200)
        self.assertEqual(claim["status"], "reimbursement_submitted")

        # Accountant reimburses.
        self._as(accountant)
        reimbursed = self._post(f"/api/fund-requests/advances/{adv.id}/reimburse",
                                {"amount": adv.amount, "method": "bank", "reference": "BANK-1"}, 200)
