...
from apps.accounts.models import (
    StaffProfile, StaffSchoolAssignment, StaffSupervisorAssignment, StaffSetupCandidate, User,
)
...
class StaffOwnershipTest(APITestCase):
    def setUp(self):
        ...
        self.district = District.objects.create(name="Staff District", region=self.region)
        self.sub_county = SubCounty.objects.create(name="Staff Sub", district=self.district)

        # An IA who performs the upload (has SCHOOL_UPLOAD perm).
        self.ia = self._user("ia@staff.test", EdifyRole.IMPACT_ASSESSMENT.value, "IA Tester")
        ...
        self.pl = self._user("pl@staff.test", EdifyRole.COUNTRY_PROGRAM_LEAD.value, "Grace PL")
    ...
    def test_staff_name_match_creates_school_assignment_and_enters_scope(self):
        ...
        self.assertTrue(
            StaffSchoolAssignment.objects.filter(staff=self.cceo_staff, school_id=school.id).exists()
        )
    ...
    def test_ambiguous_staff_name_does_not_auto_assign(self):
        ...
        self._user("cc2@staff.test", EdifyRole.CCEO.value, "Ojok Amos")  # second "Ojok Amos"
        StaffProfile.objects.create(user=User.objects.get(email="cc2@staff.test"), title="CCEO")
        body = (
            "Staff Name,School ID,School Name,District\n"
    ...
    def test_non_field_staff_not_auto_linked(self):
        ...
        self._user("ia2@staff.test", EdifyRole.IMPACT_ASSESSMENT.value, "Unique IA Name")
        body = (
            "Staff Name,School ID,School Name,District\n"
        ...
        self.assertEqual(school.account_owner_status, "unmatched")  # IA is not field staff
    ...
    def test_admin_create_user_from_candidate_links_all_schools(self):
        ...
        res = self.client.post(
            ...
            {"email": "newcceo@staff.test", "role": "CCEO", "phone": "+256700000"}, format="json",
        )
        self.assertEqual(res.status_code, 200, res.content)
        ...
        for sid in ("SCH-CAND-1", "SCH-CAND-2"):
            ...
            self.assertTrue(StaffSchoolAssignment.objects.filter(staff=new_staff, school_id=school.id).exists())
    ...
    def test_cd_assigns_supervisor_and_pl_team_scope_grows(self):
        ...
        school = School.objects.create(
            school_id="SCH-SUP-1", name="Sup Primary", region=self.region, district=self.district,
        )
        ...
        res = self.client.post(
            ...
            {"supervisorId": self.pl_staff.id, "reason": "PL manages CCEO"}, format="json",
        )
        self.assertEqual(res.status_code, 200, res.content)
        self.assertTrue(
            StaffSupervisorAssignment.objects.filter(supervisee=self.cceo_staff, supervisor=self.pl_staff).exists()
        )
        ...
        pl_principal = type("P", (), {
            "active_role": self.pl.active_role, "user_id": self.pl.id,
            "staff_profile_id": self.pl_staff.id, "id": self.pl.id,
        })()
    ...
    def test_cd_sets_project_manager(self):
        proj = Project.objects.create(code="SP-TEST", name="Test Project", category="pilot")
        self._as(self.admin)
        res = self.client.patch(
            f"/api/special-projects/{proj.id}",
            {"managerStaffId": self.pl_staff.id}, format="json",
        )
        self.assertEqual(res.status_code, 200, res.content)
