...
class PerformanceEngineTest(TestCase):
    def setUp(self):
        ...
        self.sub_county = SubCounty.objects.create(
            name="Perf Sub", district=self.district
        )

        self.cceo = User.objects.create_user(
            email="cceo@perf.test",
            name="Perf CCEO",
    ...
    def _visit(
        self, *, status="ia_verified", evidence="accepted", code="SV-1", days_ago=10
    ):
    ...
    def test_ssa_with_8_scores_counts(self):
        ...
        for interv in SsaIntervention:
            SsaScore.objects.create(
                ssa_record=rec, intervention=interv.value, score=6.5
            )
        m = staff_metrics(self.staff.id, self.fy, self.fy_start, self.fy_end)
    ...
    def test_drilldown_count_equals_card_count(self):
        ...
        m = staff_metrics(self.staff.id, self.fy, self.fy_start, self.fy_end)
        dd = drilldown(
            self.staff.id, "school_visits", self.fy, self.fy_start, self.fy_end
        )
