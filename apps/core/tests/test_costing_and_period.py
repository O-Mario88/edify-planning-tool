...
class CostingBudgetPeriodTest(APITestCase):
    ...
    def _make_visit(self, month=7, week=2):
        return self._post("/api/planning/schedule-school-visit", {
            "schoolId": "COST-SCH",
            "scheduledDate": f"2026-0{month}-10T09:00:00+03:00",
            "plannedMonth": month, "plannedWeek": week,
    ...
    def test_reschedule_replaces_budget_lines_and_moves_period(self):
        ...
        self.assertEqual({l.id for l in lines_before} & {l.id for l in lines_after}, set())
        self.assertGreater(new_total, old_total, "reschedule should re-price at the new rate")
    ...
    def test_fund_request_total_equals_line_item_sum(self):
        ...
        expected = (
            sum(l.amount for l in ActivityScheduleCostLine.objects.filter(activity_id=a1["id"]))
            + sum(l.amount for l in ActivityScheduleCostLine.objects.filter(activity_id=a2["id"]))
        )
        self.assertGreater(expected, 0)

        fr = self._post("/api/fund-requests", {"fy": "2026", "period": "monthly", "month": 7}, 201)
    ...
    def test_cost_missing_activity_blocks_fund_request(self):
        ...
        r = self.client.post("/api/fund-requests", {"fy": "2026", "period": "monthly", "month": 7}, format="json")
