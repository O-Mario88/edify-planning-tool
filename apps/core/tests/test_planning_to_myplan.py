...
def _activity_ids(context):
    ...
    keys = [
        "due_today", "this_week", "partner_monitoring", "returned_needs_correction",
        "waiting_on_me", "waiting_on_approval", "school_visits",
        "cluster_trainings", "cluster_meetings", "upcoming",
    ]
    ids = set()
    for k in keys:
...
class PlanningToMyPlanFlowTest(TestCase):
    def setUp(self):
        ...
        self.school = School.objects.create(
            school_id="S-1", name="Test School", region=self.region,
            district=self.district, enrollment=200, school_type="client",
        )

    def _cceo_with_profile(self, email="cceo@plan.test"):
        u = User.objects.create_user(
            email=email, name=email.split("@")[0].title(),
            roles=["CCEO"], active_role="CCEO", password="x", is_active=True,
        )
    ...
    def test_staff_activity_appears_in_my_plan_with_staffprofile(self):
        ...
        activity = Activity.objects.create(
            ...
            school=self.school, fy=fy, quarter="Q1",
            responsible_staff_id=profile.id,
            status="scheduled",
            scheduled_date=timezone.now(), planned_date=timezone.now().date(),
            delivery_type="staff",
        )
    ...
    def test_staff_activity_appears_in_my_plan_without_staffprofile(self):
        ...
        user = User.objects.create_user(
            email="no-profile@plan.test", name="No Profile",
            roles=["CCEO"], active_role="CCEO", password="x", is_active=True,
        )
    ...
    def test_partner_delivery_attribution_logic(self):
        ...
        a_partner = Activity.objects.create(
            activity_type="school_visit", school=self.school,
            fy=get_operational_fy(), quarter="Q1",
            responsible_staff_id=None,
            monitored_by_staff_id=principal_owner_id,
            assigned_partner_id="PARTNER-1",
            delivery_type="partner", status="assigned_to_partner",
            planned_date=date.today(),
        )
        self.assertEqual(a_partner.monitored_by_staff_id, profile.id,
        ...
        user_no_profile = User.objects.create_user(
            email="no-profile2@plan.test", name="No Profile",
            roles=["CCEO"], active_role="CCEO", password="x", is_active=True,
        )
        principal_owner_id2 = user_no_profile.staff_profile_id or user_no_profile.user_id
        self.assertEqual(principal_owner_id2, user_no_profile.id,
