...
METRIC_REGISTRY: tuple[MetricSpec, ...] = (
    ...
    MetricSpec(
        ...
        source_models=("activities.Activity",),
        numerator="Staff submissions awaiting IA with a Salesforce ID, excluding the reader's own work (partner work waits in Partner Evidence)",
        date_basis=DateBasis.RECORD_CREATED,
        period=Period.ALL_TIME,
        scope="The verifier's country; the Country Director's for IA officers' own work",
    ...
    MetricSpec(
        ...
        definition="Share of paired schools whose mean SSA score improved under the IA change rule (apps.ssa.change_rules: the published measurement rule for each domain in the school's country; any movement where no threshold is approved; readings under 120 days apart not compared).",
        question="How widespread is meaningful school improvement?",
        category=Category.PROGRESS,
        unit=Unit.PERCENT,
        service="apps.analytics.impact_engine.build_dashboard",
        source_models=("schools.School", "ssa.SsaRecord"),
        numerator="Paired schools classified improved by apps.ssa.change_rules",
