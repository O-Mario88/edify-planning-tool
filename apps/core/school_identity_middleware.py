    ...
    def __call__(self, request):
        ...
        rows = list(
            School.objects.filter(
                Q(pk__in=values) | Q(school_id__in=values)
        ...
        def replace(match):
            ...
            return str(
                format_html(
                    '<span class="school-list-id" title="School ID">{}</span>',
                    code or "ID unavailable",
                )
            )

        html = response.content.decode(response.charset)
        html = re.sub(
            r'<span data-school-code-token="([a-f0-9]{32})"></span>', replace, html
        )
