...
class SchoolIdentityTests(TestCase):
    ...
    def render_response(self, markup, data=None):
        ...
        return SchoolIdentityMiddleware(
        ...
        rows = [
            {
                ...
                "issue": "Low SSA",
                "shipping_address": "Plot 7",
                "recommended_activity_label": "Schedule visit",
            }
        ]
        response = SchoolIdentityMiddleware(
            lambda req: HttpResponse(
                render_to_string(
        ...
        class Headers(HTMLParser):
            ...
            def handle_data(self, data):
                if self.active and data.strip():
                    self.headers.append(data.strip().lower())
