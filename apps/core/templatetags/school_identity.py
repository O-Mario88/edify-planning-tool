...
def school_identity(context, name, reference=None, code=None):
    ...
    identifier = (format_html('<span class="school-list-id" title="School ID">{}</span>', code)
                  if code else school_code(context, reference))
    return format_html('<span class="school-list-identity">{} <span class="school-list-name">{}</span></span>', identifier, name or 'School not recorded')
