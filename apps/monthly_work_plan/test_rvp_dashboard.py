...
class RVPDashboardTest(TestCase):
    def setUp(self):
        ...
        self.rvp, _ = self._staff("rvp@r.org", "Regional VP",
                                  EdifyRole.REGIONAL_VICE_PRESIDENT.value)
        self.cd, _ = self._staff("cd@r.org", "Country Dir",
                                 EdifyRole.COUNTRY_DIRECTOR.value)
        self.cceo, self.cceo_sp = self._staff("cc@r.org", "Field Cceo",
                                              EdifyRole.CCEO.value)
        self.school = School.objects.create(
            school_id="RVP-S1", name="RVP School", region=self.region,
            district=self.district, enrollment=300, current_fy_ssa_status="done")

        self.submitted = MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key=f"{FY}-07", country_id="Uganda",
            status="submitted_to_rvp", program_total=4_000_000,
            admin_total=1_000_000, total_amount=5_000_000, activity_count=12,
            submitted_at=timezone.now(), submitted_by_user_id=self.cd.id)
        self.draft = MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key=f"{FY}-08", country_id="Uganda",
            status="draft_generated", total_amount=2_000_000)
        self.foreign = MonthlyWorkPlanBudget.objects.create(
            fy=FY, month_key=f"{FY}-09", country_id="Kenya",
            status="submitted_to_rvp", total_amount=9_000_000)

        self.annual = CountryAnnualBudget.objects.create(
            fy=FY, country_id="Uganda", program_total=80_000_000,
            admin_total=20_000_000, total_amount=100_000_000,
            target_schools=200, target_activities=900)
    ...
    def _completed_activity(self, sf="SF-R1", status="ia_verified", atype="school_visit"):
        a = Activity.objects.create(
            school=self.school, activity_type=atype,
            delivery_type="staff", status=status,
            responsible_staff_id=self.cceo_sp.id, fy=FY, quarter="Q4",
            planned_date=date.today(), salesforce_activity_id=sf,
            teachers_attended=5)
        ActivityScheduleCostLine.objects.create(
            activity=a, cost_setting_key="transport", label="Transport",
            unit_cost=100_000, amount=100_000)
        return a
    ...
    def test_rvp_dashboard_has_no_field_planning_actions(self):
        ...
        for banned in ("Schedule Visit", "Schedule Training", "Start Activity",
                       "Upload Evidence", "Enter SF ID", "Disburse"):
    ...
    def test_rvp_country_ranking_uses_verified_data(self):
        ...
        self.assertIn(row["status"], ("Strong", "On Track", "Watch",
                                      "High Risk", "Critical"))
    ...
    def test_rvp_can_approve_valid_country_monthly_budget(self):
        ...
        self.assertTrue(RVPApprovalDecision.objects.filter(
            decision_type="monthly_budget", subject_id=self.submitted.id,
            action="approve").exists())
    ...
    def test_rvp_can_approve_country_annual_budget(self):
        ...
        self.assertTrue(RVPApprovalDecision.objects.filter(
            decision_type="annual_budget", subject_id=b.id,
            action="approve").exists())
    ...
    def test_special_project_impact_uses_annual_ssa(self):
        ...
        SsaRecord.objects.create(
            school=self.school, fy=PREV, quarter="Q1", average_score=5.0,
            verification_status="confirmed",
            date_of_ssa=date(int(PREV) - 1, 11, 1), uploaded_by="t")
        SsaRecord.objects.create(
            school=self.school, fy=FY, quarter="Q1", average_score=7.0,
            verification_status="confirmed",
            date_of_ssa=date(int(FY) - 1, 11, 1), uploaded_by="t")
        d = self._dash()
        row = next(p for p in d["projects"] if p["id"] == self.project.id)
    ...
    def test_special_project_decision_creates_audit(self):
        ...
        self.assertTrue(RVPApprovalDecision.objects.filter(
            decision_type="special_project", subject_id=self.project.id,
            action="scale").exists())
    ...
    def test_resource_reallocation_requires_budget_amendment(self):
        ...
        with self.assertRaises(BadRequest):
            update_annual_budget(self.annual.id,
                                 {"program_total": 70_000_000}, self.rvp)
        ...
        rvp_annual_decide(self.annual.id, "return",
                          {"note": "Reallocate toward core schools"}, self.rvp)
        updated = update_annual_budget(self.annual.id,
                                       {"program_total": 70_000_000}, self.cd)
    ...
    def test_strategy_note_creates_todo(self):
        ...
        create_strategy_note({
            "priority": "School Leadership", "scope": "Uganda",
    ...
    def test_rvp_htmx_endpoints_enforce_scope(self):
        ...
        self.assertEqual(c.post(f"/rvp/annual/{self.annual.id}/action",
                                {"action": "approve"}).status_code, 403)
        self.assertEqual(c.post("/rvp/strategy-note",
