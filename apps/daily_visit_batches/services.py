...
def _resolve_group(district_ids: set[str]):
    ...
    return (
        ...
        .annotate(
            n=Count("members__district_id", distinct=True, filter=Q(members__district_id__in=district_ids))
        )
...
def schedule_visits(
    *, school_ids: list[str], scheduled_date: date, activity_common_fields: dict,
    reason: str | None, principal,
) -> dict:
    ...
    schools = list(
        School.objects.filter(school_id__in=school_ids, deleted_at__isnull=True).select_related("district")
    )
    ...
    with transaction.atomic():
        ...
        existing_activities = (
            list(batch.activities.filter(deleted_at__isnull=True).exclude(status="cancelled").select_related("school"))
            if batch else []
        )
        existing_district_ids = {
            a.school.district_id for a in existing_activities if a.school_id and a.school.district_id
        }
        ...
        if total_after > target:
            raise BadRequest(f"You can only schedule {target} for this date, please choose another date.")
        ...
        if not batch:
            batch = DailyVisitBatch.objects.create(
                ...
                secondary_district_group=(
                    ...
                    if incoming_district_type == "secondary" and len(all_district_ids) > 1
                    else None
                ),
            )
        ...
        Activity.objects.filter(id__in=[a["id"] for a in new_activities]).update(daily_visit_batch=batch)
...
def reschedule_within_batch(*, activity, new_date: date, reason: str | None, principal) -> None:
    ...
    if not school or not school.district_id:
        raise BadRequest("This activity has no school/district on file — cannot batch-price it.")
    ...
    with transaction.atomic():
        ...
        if batch and _is_locked(responsible_user_id, new_date):
            raise BadRequest("This date's visits have already left draft status. Choose another date.")
        ...
        if not batch:
            batch = DailyVisitBatch.objects.create(
                responsible_user=responsible_user_id, visit_date=new_date, district_type=incoming_type,
                secondary_district_group=(
                    _resolve_group(all_district_ids)
                    if incoming_type == "secondary" and len(all_district_ids) > 1 else None
                ),
            )
        if effective_reason and effective_reason != batch.reason:
            batch.reason = effective_reason
...
def _recalculate_and_write_lines(batch: DailyVisitBatch, catalogue, responsible_user_id: str) -> None:
    ...
    from apps.budget.costing_service import _rate_card, active_catalogue, apply_to_activity
    ...
    activities = list(
        batch.activities.filter(deleted_at__isnull=True).exclude(status="cancelled").order_by("id")
    )
    n = len(activities)
    ...
    batch.required_target_snapshot = catalogue.required_school_visits_per_day if catalogue else 5
    batch.save()
    ...
    for activity, alloc in zip(activities, allocations):
        lines = [
            CostLine(
                ...
                key=key, unit=amount, qty=1, amount=amount, missing=False,
            )
            for key, amount in alloc.items()
        ]
        cost = ActivityCost(amount=sum(alloc.values()), lines=lines, cost_missing=False, missing_items=[])
        apply_to_activity(
            activity, {"fy": activity.fy}, responsible_user_id=responsible_user_id, precomputed_cost=cost
        )
