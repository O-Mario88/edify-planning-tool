...
class DailyVisitBatchTestCase(TestCase):
    ...
    def test_single_school_batch_of_one(self):
        ...
        # transport + lunch, and nothing else (owner, 2026-09-12)
        self.assertEqual(len(lines), 2)
        self.assertEqual(sum(l.amount for l in lines), expected_pool)

    # ── 1a. The owner's arithmetic: secondary 5-school split = 120,000 each ──
    def test_secondary_five_school_split_equals_120000_each(self):
        """The owner's formula (2026-09-12): a secondary day is transport
        330,000 + lunch 30,000 + breakfast 40,000 + dinner 50,000 +
        accommodation 150,000 = 600,000, divided by the schools planned that
        day. Five schools pay exactly 120,000 each and the pool reconciles to
        the shilling. Breakfast is part of every secondary day now, so a card
        without it cannot price one."""
        from apps.core.exceptions import BadRequest
        ...
        self.assertEqual(sum(pool.values()), 600000)
        allocations = allocate_pool(pool, 5)
        per_school = [sum(a.values()) for a in allocations]
        self.assertEqual(per_school, [120000, 120000, 120000, 120000, 120000])
        # Exact reconciliation — no shillings created or lost to rounding.
        self.assertEqual(sum(per_school), 600000)
        without_breakfast = {k: v for k, v in rates.items() if "breakfast" not in k}
        with self.assertRaises(BadRequest):
            compute_daily_pool(without_breakfast, "secondary")
...
class OneMissionCostPerDayTest(DailyVisitBatchTestCase):
    ...
    def test_rescheduling_training_rebalances_both_days(self):
        ...
        self.assertEqual(
            ...
            # Half of the day's transport plus staff lunch.
            155000,
        )
    ...
    def test_secondary_daily_components_are_charged_once_across_schools(self):
        ...
        CostSetting.objects.filter(key="secondary_breakfast_per_day").update(
            unit_cost=15001, approved_minimum=7001
        )
        ...
        for key, rate in [
            (k, 15001 if k == "secondary_breakfast_per_day" else r)
            for k, r in SECONDARY_RATES
        ]:
            self.assertEqual(
                ActivityScheduleCostLine.objects.filter(
...
class ConfiguredStaffDailyShareTest(DailyVisitBatchTestCase):
    def _four_visits(self, *, secondary=False):
        ...
        self.staff_profile.primary_district_id = (
            self.secondary_district_a.id if secondary else self.primary_district.id
        )
        self.staff_profile.save(update_fields=["primary_district_id"])
        # Deliberately contrary global classification: the USER's home wins.
        self.primary_district.district_type = "primary" if secondary else "secondary"
        self.primary_district.save(update_fields=["district_type"])
        rates = {
            "primary_transport_per_day": 300000,
            "secondary_transport_per_day": 300000,
            "lunch_per_day": 30000,
            "secondary_breakfast_per_day": 20000,
            "secondary_overnight_dinner_per_day": 40000,
            "secondary_accommodation_per_night": 100000,
        }
    ...
    def test_primary_330000_divided_between_four_visits(self):
        self._assert_shares(False, 82500)

    def test_secondary_490000_divided_between_four_visits(self):
        self._assert_shares(True, 122500)

    def test_repair_attaches_legacy_core_visit_and_resplits_every_member(self):
        from io import StringIO
        from django.core.management import call_command
        from apps.budget.models import ActivityCostSnapshot
    ...
    def test_user_configuration_change_reprices_editable_day(self):
        from io import StringIO
        from django.core.management import call_command
        from apps.daily_visit_batches.services import batch_needs_repricing
