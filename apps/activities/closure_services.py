...
class ClosureEligibilityService:
    ...
    def evaluate(activity: Activity) -> tuple[ClosureChecklist, list[ClosureBlocker]]:
        with transaction.atomic():
            ...
            executed = activity.status not in ["not_planned", "planned", "scheduled", "assigned_to_partner", "partner_scheduled"]
            
            # Check 2: Evidence Uploaded
            evidence_uploaded = EvidenceRecord.objects.filter(activity=activity, quarantined=False).exists()
            
            # Check 3: Salesforce ID entered
            salesforce_id_entered = bool(activity.salesforce_activity_id)
            
            # Check 4: IA Verified
            ia_verified = activity.status in ["ia_verified", "closed", "accountant_confirmed"]
            
            # Check 5: Finance Required
            finance_required = activity.schedule_cost_lines.exists() or activity.delivery_type == "partner"
            ...
            if finance_required:
                netsuite_id_entered = NetSuiteExpenseRecord.objects.filter(activity=activity).exists()
            ...
            audit_trail_saved = ActivityTimelineEvent.objects.filter(activity=activity).exists()
            ...
            if not executed:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="Activity not executed", responsible_role="CCEO"))
            if not evidence_uploaded:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="Evidence Missing", responsible_role="CCEO"))
            if not salesforce_id_entered:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="Activity SF ID Missing", responsible_role="CCEO"))
            if not ia_verified:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="IA not verified", responsible_role="ImpactAssessment"))
            if finance_required and not accounts_cleared:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="Accounts not cleared", responsible_role="Accountant"))
            if finance_required and not netsuite_id_entered:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="NetSuite ID missing", responsible_role="Accountant"))
            if not analytics_published:
                blockers.append(ClosureBlocker.objects.create(activity=activity, blocking_reason="Analytics not published", responsible_role="ImpactAssessment"))
    ...
    def is_eligible(activity: Activity) -> bool:
        ...
        required_pass = (
            checklist.activity_executed and 
            checklist.evidence_uploaded and 
            checklist.salesforce_id_entered and 
            checklist.ia_verified and 
            (not checklist.finance_required or (checklist.accounts_cleared and checklist.netsuite_id_entered))
        )
...
class ActivityClosureService:
    ...
    @staticmethod
    def close(activity: Activity, closed_by: str = "system", bypass_checks: bool = False) -> ActivityClosure:
        # Check eligibility first
        if not bypass_checks and not ClosureEligibilityService.is_eligible(activity):
            raise ValueError("Activity does not meet final closure checklist requirements.")
        ...
        with transaction.atomic():
            ...
            budget_total = activity.schedule_cost_lines.aggregate(s=Sum("amount"))["s"] or 0
            disb_total = Disbursement.objects.filter(activity=activity).aggregate(s=Sum("amount_disbursed"))["s"] or 0
            ...
            CompletedActivitySnapshot.objects.update_or_create(
                ...
                defaults={
                    ...
                    "evidence_count": EvidenceRecord.objects.filter(activity=activity).count(),
...
class ActivityReopenService:
    ...
    @staticmethod
    def reopen(activity: Activity, reason: str, category: str, user_id: str) -> ActivityReopenRequest:
        with transaction.atomic():
            ...
            activity.status = "ia_verified"  # Revert to verified so it can be re-cleared/fixed
            activity.save(update_fields=["status", "updated_at"])
            
            # Update closure record status
            ActivityClosure.objects.filter(activity=activity).update(
                status="reopened",
                notes=f"Reopened by {user_id}. Reason: {reason}"
            )
...
class AuditTrailService:
    ...
    @staticmethod
    def log_event(activity: Activity, event_name: str, actor_id: str, actor_role: str, description: str = "") -> ActivityTimelineEvent:
