...
class IAVerification(TimeStampedModel):
    ...
    activity = models.OneToOneField(
        "activities.Activity", on_delete=models.CASCADE, related_name="ia_verification"
    )
...
class VerificationChecklist(TimeStampedModel):
    ...
    verification = models.OneToOneField(
        IAVerification, on_delete=models.CASCADE, related_name="checklist"
    )
...
class VerificationComment(TimeStampedModel):
    ...
    verification = models.ForeignKey(
        IAVerification, on_delete=models.CASCADE, related_name="comments"
    )
...
class VerificationDecision(TimeStampedModel):
    ...
    id = CuidField()
    verification = models.ForeignKey(
        IAVerification, on_delete=models.CASCADE, related_name="decisions"
    )
...
class ReturnedReason(TimeStampedModel):
    ...
    id = CuidField()
    verification = models.ForeignKey(
        IAVerification, on_delete=models.CASCADE, related_name="returned_reasons"
    )
...
class DuplicateActivity(TimeStampedModel):
    ...
    activity = models.ForeignKey(
        "activities.Activity", on_delete=models.CASCADE, related_name="duplicate_flags"
    )
    ...
    status = models.CharField(
        max_length=32, default="potential"
    )  # potential | ignored | resolved | flagged
