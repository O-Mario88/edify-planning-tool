                ...
                for row in reader:
                    try:
                        ...
                        district = District.objects.filter(
                            name__iexact=row["district"].strip()
                        ).first()
                        if not region or not district:
                            skipped += 1
                            self.stderr.write(
                                f"  skip {row.get('schoolId')}: region/district not found"
                            )
                            continue
                        sub_county = None
                        if row.get("subCounty"):
                            sub_county = SubCounty.objects.filter(
                                name__iexact=row["subCounty"].strip(), district=district
                            ).first()
                        defaults = {
                            "name": row["name"].strip(),
                            "region": region,
                            ...
                            or SchoolType.CLIENT,
                            "enrollment": int(row["enrollment"])
                            if row.get("enrollment")
                            else None,
                            "school_phone": (row.get("schoolPhone") or "").strip()
                            or None,
                            "primary_contact_name": (
                                row.get("primaryContactName") or ""
                            ).strip()
                            or None,
                            "primary_contact_phone": (
                                row.get("primaryContactPhone") or ""
                            ).strip()
                            or None,
                            "shipping_address": (
                                row.get("shippingAddress") or ""
                            ).strip()
                            or None,
                            "source": source,
                            "environment": env_tag,
