...
class ClusterSetupTest(APITestCase):
    def setUp(self):
        ...
        self.sub_county1 = SubCounty.objects.create(name="Ntunga", district=self.district)
        self.sub_county2 = SubCounty.objects.create(name="Ntunga South", district=self.district)
        ...
        self.profile = StaffProfile.objects.create(user=self.user, title="CCEO", id="STF-001")
        ...
        ClusterSubCounty.objects.create(cluster=self.cluster1, sub_county=self.sub_county1)
        ...
        ClusterSubCounty.objects.create(cluster=self.cluster2, sub_county=self.sub_county2)
    ...
    def test_auto_classification_on_create(self):
        ...
        self.assertTrue(
            SchoolClusterAssignment.objects.filter(school=school, cluster=self.cluster1).exists()
        )
    ...
    def test_real_time_reclassification_on_update(self):
        ...
        self.assertTrue(
            SchoolClusterAssignment.objects.filter(school=school, cluster=self.cluster2).exists()
        )
    ...
    def test_reclassification_to_unclustered(self):
        ...
        unclustered_sub = SubCounty.objects.create(name="Ntunga East", district=self.district)
    ...
    def test_dynamic_cluster_metrics_and_feed(self):
        ...
        school1 = School.objects.create(
            school_id="SCH-101", name="Alpha Acad", region=self.region,
            district=self.district, sub_county=self.sub_county1, school_type="client",
            current_fy_ssa_status="done"
        )
        school2 = School.objects.create(
            school_id="SCH-102", name="Beta Acad", region=self.region,
            district=self.district, sub_county=self.sub_county1, school_type="client",
            current_fy_ssa_status="done"
        )
        ...
        r1 = SsaRecord.objects.create(
            school=school1, date_of_ssa=now, fy="2026", quarter="Q1",
            average_score=6.0, verification_status="confirmed"
        )
        r2 = SsaRecord.objects.create(
            school=school2, date_of_ssa=now, fy="2026", quarter="Q1",
            average_score=8.0, verification_status="confirmed"
        )
        ...
        self.assertEqual(cluster_data["weakest"]["intervention"], SsaIntervention.FINANCIAL_HEALTH.value)
