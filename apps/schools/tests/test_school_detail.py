...
class SchoolDetailAndAssignmentOptionsTest(APITestCase):
    ...
    def test_assignment_options_for_unassigned_school(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {issue_access_token(self.user.id, self.user.active_role)}")
        
        response = self.client.get(f"/api/assignment/options?schoolId={self.school.school_id}")
