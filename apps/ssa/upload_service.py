...
def upload_ssa_file(file, principal) -> dict:
    ...
    [i for i in M.ALL_INTERVENTIONS if i in field_index]
    ...
    for row_number, cells in data_rows:
        {f: _value(field_index, f, cells) for f in field_index}
    ...
    if len(data_rows) == 0:
        ...
        message = (
            "Nothing validated — all row(s) failed validation. See the errors below."
        )
