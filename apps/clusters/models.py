...
class Cluster(SoftDeleteModel):
    ...
    region = models.ForeignKey("geography.Region", on_delete=models.RESTRICT, related_name="clusters")
    district = models.ForeignKey("geography.District", on_delete=models.RESTRICT, related_name="clusters")
    # PRIMARY sub-county (first selected); display + back-compat.
    sub_county = models.ForeignKey(
        "geography.SubCounty", on_delete=models.SET_NULL, null=True, blank=True, related_name="clusters"
    )
    sub_county_name = models.CharField(max_length=255, null=True, blank=True)  # display fallback
    cluster_type = models.CharField(
        max_length=16, choices=ClusterType.choices, default=ClusterType.MIXED
    )
    status = models.CharField(
        max_length=32, choices=ClusterRecordStatus.choices, default=ClusterRecordStatus.ACTIVE
    )
...
class ClusterSubCounty(TimeStampedModel):
    ...
    id = CuidField()
    cluster = models.ForeignKey(Cluster, on_delete=models.CASCADE, related_name="covered_sub_counties")
    sub_county = models.ForeignKey("geography.SubCounty", on_delete=models.CASCADE, related_name="cluster_coverage")

    class Meta:
        db_table = "cluster_sub_county"
        constraints = [
            models.UniqueConstraint(fields=["cluster", "sub_county"], name="uniq_cluster_subcounty"),
        ]
...
class SchoolClusterAssignment(TimeStampedModel):
    ...
    id = CuidField()
    school = models.ForeignKey("schools.School", on_delete=models.CASCADE, related_name="cluster_assignments")
    cluster = models.ForeignKey(Cluster, on_delete=models.CASCADE, related_name="assignments")
    assigned_by = models.CharField(max_length=30)  # userId

    class Meta:
        db_table = "school_cluster_assignment"
        constraints = [
            models.UniqueConstraint(fields=["school", "cluster"], name="uniq_school_cluster"),
        ]
