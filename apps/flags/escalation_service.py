...
def visible_to(principal):
    ...
    if role == EdifyRole.COUNTRY_DIRECTOR.value:
        ...
        q |= oversight
        # The country's board, not the raiser's own list: a second CD
        # account, or the same director under another account, must still
        # see what the country has escalated upward.
        upward = Q(addressed_role=EscalationAddressee.REGIONAL_VICE_PRESIDENT.value)
        if country:
            upward &= Q(country_id=country)
        q |= upward
    return qs.filter(q)
