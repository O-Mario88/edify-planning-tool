...
class FieldDebriefWeeklyRollupService:
    ...
    def for_user(principal, week_start: date | None = None) -> dict:
        ...
        qs = FieldDebriefService.scoped_queryset(principal, {"mine": True}).filter(
            date__date__gte=start, date__date__lte=end
        )
        rows = list(
            qs.prefetch_related("challenges", "support_requests", "peer_solutions")
        )
        return {
            "week_start": start,
            "week_end": end,
            ...
            "activities_completed": sum(
                1
                for r in rows
                if r.completion_status == CompletionStatus.COMPLETED_AS_PLANNED
            ),
            "activities_unsuccessful": sum(
                1 for r in rows if r.completion_status == CompletionStatus.UNSUCCESSFUL
            ),
            "schools_visited": len({s for r in rows for s in r.linked_school_ids}),
            "interventions_addressed": sorted(
                {i for r in rows for i in r.intervention_tags}
            ),
            "top_challenges": _top_challenges(rows),
            "open_commitments": sum(
                1 for r in rows for c in r.commitments.all() if c.status == "open"
            ),
    ...
    def for_pl(principal, week_start: date | None = None) -> dict:
        start, end = _week_bounds(week_start)
        qs = FieldDebriefService.scoped_queryset(principal, {}).filter(
            date__date__gte=start, date__date__lte=end
        )
        rows = list(qs)
        ...
        return {
            ...
            "actions_open": sum(
                r.actions.exclude(status__in=("resolved", "closed")).count()
                for r in rows
            ),
        }
...
def _recurring_issue_count(rows) -> int:
    ...
    return (
        DailyDebriefInsight.objects.filter(status="open")
        .filter(Q(scope_id__in=school_ids) | Q(scope_id__in=staff_ids))
        .count()
    )
...
def field_debrief_intelligence_summary(principal, days: int = 30) -> dict:
    ...
    return {
        ...
        "action_required": sum(
            1 for r in rows if r.status == DebriefStatus.ACTION_REQUIRED
        ),
