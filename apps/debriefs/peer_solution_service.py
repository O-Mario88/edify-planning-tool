...
class PeerSolutionService:
    @staticmethod
    def propose(principal, debrief_id: str, *, suggestion: str, related_experience: str = "") -> DailyDebriefPeerSolution:
        ...
        solution = DailyDebriefPeerSolution.objects.create(
            debrief=debrief, author_user_id=principal.user_id,
            suggestion=suggestion.strip(), related_experience=related_experience or None,
        )
        from apps.notifications.services import WorkflowNotificationService

        WorkflowNotificationService.trigger(
            event_type="field_debrief_peer_solution", category="field_debrief", priority="normal",
            title="A peer proposed a solution on your debrief",
            body=suggestion.strip()[:200], context_type="field_debrief", context_id=debrief.id,
    ...
    def endorse(principal, solution_id: str) -> DailyDebriefPeerSolution:
        solution = DailyDebriefPeerSolution.objects.select_related("debrief").filter(id=solution_id).first()
        if not solution:
        ...
        if principal.user_id not in solution.endorsed_by_user_ids:
            solution.endorsed_by_user_ids = [*solution.endorsed_by_user_ids, principal.user_id]
            ...
            if solution.author_user_id and solution.author_user_id != principal.user_id:
                ...
                WorkflowNotificationService.trigger(
                    event_type="field_debrief_peer_solution_endorsed", category="field_debrief", priority="normal",
                    title="Your peer solution was endorsed",
                    body=solution.suggestion[:200], context_type="field_debrief", context_id=solution.debrief_id,
                    recipients=[solution.author_user_id],
                )
        return solution

    @staticmethod
    def pl_classify(principal, solution_id: str, *, classification: str) -> DailyDebriefPeerSolution:
        ...
        solution.save(update_fields=["pl_classification", "pl_classified_by_user_id", "pl_classified_at", "status"])
        if solution.author_user_id and solution.author_user_id != principal.user_id:
            from apps.notifications.services import WorkflowNotificationService

            WorkflowNotificationService.trigger(
                event_type="field_debrief_peer_solution_classified", category="field_debrief", priority="normal",
