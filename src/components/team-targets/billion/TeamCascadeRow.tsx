...
import {
  teamCascade,
  type TeamCascadeStatus,
  type TeamCascadeTile,
  type TeamCascadeTone,
} from "@/lib/team-targets-billion-mock";
import { cn } from "@/lib/utils";

// 4-tier team cascade — FY → Quarter → Month → This Week. The
// "amount" is a team-aggregate activity count (e.g. "4,280" of
// "4,520 activities").
export function TeamCascadeRow() {
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4">
      {teamCascade.map((t) => (
        <CascadeTile key={t.label} t={t} />
      ))}
    </section>
  );
}
...
const STATUS_TONE: Record<TeamCascadeStatus, { chip: string; ring: string }> = {
...
const INDEX_TONE: Record<TeamCascadeStatus, string> = {
  "On Track":        "bg-emerald-500",
  "Slightly Behind": "bg-amber-500",
  "Critical":        "bg-rose-500",
};

const PACE_COLOR: Record<TeamCascadeTone, string> = {
  good:  "text-emerald-700",
  watch: "text-amber-700",
  warn:  "text-rose-700",
};

function CascadeTile({ t }: { t: TeamCascadeTile }) {
