...
const REPORTS: ReportTile[] = [
  ...
  { title: "Verified Impact",       description: "Leaderboard of verified work across categories + most-improved.",             href: "/leaderboard",         Icon: Trophy,        iconBg: "bg-yellow-100",  iconText: "text-yellow-700",  roles: ["CCEO", "CountryProgramLead", "CountryDirector", "RVP", "HumanResource", "ImpactAssessment", "Admin"] },
        ...
        <ul className="divide-y divide-[var(--color-edify-divider)]">
          {recentReports.map((r) => (
            <li key={r.id} className="py-2.5 flex items-center gap-3">
              ...
              <a
                href={r.href}
                className="h-8 w-8 rounded-md border border-[var(--color-edify-border)] grid place-items-center hover:bg-[var(--color-edify-soft)]/40 shrink-0"
                aria-label={`Download ${r.title}`}
              >
                <Download size={13} className="text-[var(--color-edify-muted)]" />
              </a>
            </li>
          ))}
        </ul>
