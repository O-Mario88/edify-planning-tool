...
| Full test suite | `manage.py test --parallel 4` | **PASS** — 5,929 tests, 0 failures, 0 skips |
...
| E2E journey census | `test_release_journey_census` | **FAIL** — 8 of 22, 2 unbuildable |
...
Suite size at HEAD: **432 test files; the runner collected and ran 5,929 tests.** The run
...
| P1 | RC-003 | 8 of 22 mandated end-to-end journeys have a real test | 21 journey tests is a work programme, not a fix. The 22 are now enumerated and the count machine-checked — see below |
...
That leaves **12 journeys that are unwritten rather than unbuildable**, which is a work
